...
namespace {
    ...
    template<typename URng, typename Estimator> sample resample(URng& rng, unsigned int resamples, std::vector<double>::iterator first, std::vector<double>::iterator last, Estimator& estimator) {
...
namespace Catch {
    ...
    std::string ExceptionTranslatorRegistry::tryTranslators() const {
        if(m_translators.empty()) { std::rethrow_exception(std::current_exception()); }
        else { return m_translators[0]->translate(m_translators.begin() + 1, m_translators.end()); }
    }
...
namespace Catch {
    ...
    void ReporterRegistry::registerListener(
        Detail::unique_ptr<EventListenerFactory> factory) { m_listeners.push_back(CATCH_MOVE(factory)); }
            ...
            void close() override {
                ...
                const bool should_wait_for_child = [&]() {
                    ...
                    if(std::find_if(
                        m_children.begin(),
                        m_children.end(),
                        [](TestCaseTracking::ITrackerPtr const& tracker) { return tracker->hasStarted(); }) != m_children.end()) { return false; }
                    ...
                    for(auto const& child : m_children) {
                        if(child->isSectionTracker() &&
                            std::find(
                                ...
                                .trimmedName()) != filters.end()) { return true; }
                    }
                    return false;
                }();
...
namespace Catch {
    ...
    void toLowerInPlace(std::string& s) { std::transform(s.begin(), s.end(), s.begin(), [](char c) { return toLower(c); }); }
    ...
    namespace TestCaseTracking {
        ...
        ITracker* ITracker::findChild(NameAndLocationRef nameAndLocation) {
            auto it = std::find_if(
                m_children.begin(),
                m_children.end(),
                [&nameAndLocation](ITrackerPtr const& tracker) { return tracker->nameAndLocation() == nameAndLocation; });
        ...
        bool ITracker::isSuccessfullyCompleted() const { return m_runState == CompletedSuccessfully; }
        ...
        std::vector<StringRef> const& SectionTracker::getFilters() const { return m_filters; }
