...
typename get_pod_type<eT>::result spop_norm::mat_norm_1(const SpMat<eT>& X) {
...
template<typename eT> inline
typename get_pod_type<eT>::result spop_norm::mat_norm_2(const SpMat<eT>& X, const typename arma_real_only<eT>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
template<typename eT> inline
typename get_pod_type<eT>::result spop_norm::mat_norm_2(const SpMat<eT>& X, const typename arma_cx_only<eT>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
template<typename eT> inline
typename get_pod_type<eT>::result spop_norm::mat_norm_inf(const SpMat<eT>& X) {
	arma_extra_debug_sigprint();
