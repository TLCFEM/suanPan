...
void spop_trimatu_ext::apply_noalias(SpMat<eT>& out, const SpMat<eT>& A, const uword row_offset, const uword col_offset) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void spop_trimatu_ext::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_trimatu_ext>& in) {
	arma_extra_debug_sigprint();
	...
	else { spop_trimatu_ext::apply_noalias(out, A, row_offset, col_offset); }
}

//

template<typename eT> inline
void spop_trimatl_ext::apply_noalias(SpMat<eT>& out, const SpMat<eT>& A, const uword row_offset, const uword col_offset) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void spop_trimatl_ext::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_trimatl_ext>& in) {
	arma_extra_debug_sigprint();
	...
	else { spop_trimatl_ext::apply_noalias(out, A, row_offset, col_offset); }
}
