...
template<typename eT> struct arma_rng::randi {
	...
	void fill(eT* mem, const uword N, const int a, const int b) {
      ...
      for(uword i=0; i<N; ++i)  { mem[i] = eT(local_i_distr(mt19937_64_instance)); }
		...
		{
			if(N == uword(1)) {
				arma_rng_cxx98::randi_fill(mem, uword(1), a, b);
				return;
			}
...
template<typename eT> struct arma_rng::randu {
	...
	void fill(eT* mem, const uword N) {
      ...
      for(uword i=0; i < N; ++i)  { mem[i] = eT( local_u_distr(mt19937_64_instance) ); }
		...
		{
			if(N == uword(1)) {
				mem[0] = eT(arma_rng_cxx98::randu_val());
				return;
			}
...
template<typename eT> struct arma_rng::randn {
	...
	void fill_simple(eT* mem, const uword N) {
      ...
      for(uword i=0; i < N; ++i)  { mem[i] = eT( local_n_distr(mt19937_64_instance) ); }
		...
		{
			if(N == uword(1)) {
				mem[0] = eT(arma_rng_cxx98::randn_val());
				return;
			}
...
template<typename T> struct arma_rng::randn<std::complex<T>> {
	...
	inline
	static
	void dual_val(std::complex<T>& out1, std::complex<T>& out2) {
#if defined(_MSC_VER)
		T a;
		T b;
#else
	...
	inline
	static
	void fill_simple(std::complex<T>* mem, const uword N) {
#if defined(ARMA_RNG_ALT)
      {
...
template<typename eT> struct arma_rng::randg {
	inline
	static
	void fill_simple(eT* mem, const uword N, const double a, const double b) {
#if defined(ARMA_USE_EXTERN_RNG)
      {
	...
	inline
	static
	void fill(eT* mem, const uword N, const double a, const double b) {
#if defined(ARMA_USE_OPENMP)
      {
