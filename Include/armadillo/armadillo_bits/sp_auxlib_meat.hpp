...
bool sp_auxlib::eigs_sym(Col<eT>& eigval, Mat<eT>& eigvec, const SpBase<eT, T1>& X, const uword n_eigvals, const form_type form_val, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
//! immediate eigendecomposition of symmetric real sparse object
template<typename eT, typename T1> inline
bool sp_auxlib::eigs_sym(Col<eT>& eigval, Mat<eT>& eigvec, const SpBase<eT, T1>& X, const uword n_eigvals, const eT sigma, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
bool sp_auxlib::eigs_sym_newarp(Col<eT>& eigval, Mat<eT>& eigvec, const SpMat<eT>& X, const uword n_eigvals, const form_type form_val, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
bool sp_auxlib::eigs_sym_newarp(Col<eT>& eigval, Mat<eT>& eigvec, const SpMat<eT>& X, const uword n_eigvals, const eT sigma, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
template<typename eT, bool use_sigma> inline
bool sp_auxlib::eigs_sym_arpack(Col<eT>& eigval, Mat<eT>& eigvec, const SpMat<eT>& X, const uword n_eigvals, const form_type form_val, const eT sigma, const eigs_opts& opts) {
...
#if defined(ARMA_USE_ARPACK)
	{
		...
		else { run_aupd_plain(n_eigvals, which, X, true /* sym, not gen */, n, tol, maxiter, resid, ncv, v, ldv, iparam, ipntr, workd, workl, lworkl, rwork, info); }
...
bool sp_auxlib::eigs_gen(Col<std::complex<T>>& eigval, Mat<std::complex<T>>& eigvec, const SpBase<T, T1>& X, const uword n_eigvals, const form_type form_val, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
//! immediate eigendecomposition of non-symmetric real sparse object
template<typename T, typename T1> inline
bool sp_auxlib::eigs_gen(Col<std::complex<T>>& eigval, Mat<std::complex<T>>& eigvec, const SpBase<T, T1>& X, const uword n_eigvals, const std::complex<T> sigma, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
bool sp_auxlib::eigs_gen_newarp(Col<std::complex<T>>& eigval, Mat<std::complex<T>>& eigvec, const SpMat<T>& X, const uword n_eigvals, const form_type form_val, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
template<typename T, bool use_sigma> inline
bool sp_auxlib::eigs_gen_arpack(Col<std::complex<T>>& eigval, Mat<std::complex<T>>& eigvec, const SpMat<T>& X, const uword n_eigvals, const form_type form_val, const std::complex<T> sigma, const eigs_opts& opts) {
...
#if defined(ARMA_USE_ARPACK)
	{
		...
		else { run_aupd_plain(n_eigvals, which, X, false /* gen, not sym */, n, tol, maxiter, resid, ncv, v, ldv, iparam, ipntr, workd, workl, lworkl, rwork, info); }
...
template<typename T, typename T1> inline
bool sp_auxlib::eigs_gen(Col<std::complex<T>>& eigval, Mat<std::complex<T>>& eigvec, const SpBase<std::complex<T>, T1>& X_expr, const uword n_eigvals, const form_type form_val, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
//! immediate eigendecomposition of non-symmetric complex sparse object
template<typename T, typename T1> inline
bool sp_auxlib::eigs_gen(Col<std::complex<T>>& eigval, Mat<std::complex<T>>& eigvec, const SpBase<std::complex<T>, T1>& X, const uword n_eigvals, const std::complex<T> sigma, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
template<typename T, bool use_sigma> inline
bool sp_auxlib::eigs_gen(Col<std::complex<T>>& eigval, Mat<std::complex<T>>& eigvec, const SpMat<std::complex<T>>& X, const uword n_eigvals, const form_type form_val, const std::complex<T> sigma, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
bool sp_auxlib::spsolve_simple(Mat<typename T1::elem_type>& X, const SpBase<typename T1::elem_type, T1>& A_expr, const Base<typename T1::elem_type, T2>& B_expr, const superlu_opts& user_opts) {
...
#if defined(ARMA_USE_SUPERLU)
	{
		...
		else if(info > int(A.n_cols)) { arma_debug_warn_level(1, "spsolve(): memory allocation failure: could not allocate ", (info - int(A.n_cols)), " bytes"); }
		else if(info < 0) { arma_debug_warn_level(1, "spsolve(): unknown SuperLU error code from gssv(): ", info); }
...
bool sp_auxlib::spsolve_refine(Mat<typename T1::elem_type>& X, typename T1::pod_type& out_rcond, const SpBase<typename T1::elem_type, T1>& A_expr, const Base<typename T1::elem_type, T2>& B_expr, const superlu_opts& user_opts) {
...
#if defined(ARMA_USE_SUPERLU)
	{
		...
		else if(info > int(A.n_cols + 1)) { arma_debug_warn_level(1, "spsolve(): memory allocation failure: could not allocate ", (info - int(A.n_cols)), " bytes"); }
		else if(info < 0) { arma_debug_warn_level(1, "spsolve(): unknown SuperLU error code from gssvx(): ", info); }
...
template<typename eT> inline
typename get_pod_type<eT>::result sp_auxlib::norm1(superlu::SuperMatrix* A) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename get_pod_type<eT>::result sp_auxlib::lu_rcond(superlu::SuperMatrix* L, superlu::SuperMatrix* U, typename get_pod_type<eT>::result norm_val) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
bool sp_auxlib::copy_to_supermatrix_with_shift(superlu::SuperMatrix& out, const SpMat<eT>& A, const eT shift) {
	arma_extra_debug_sigprint();
...
void sp_auxlib::destroy_supermatrix(superlu::SuperMatrix& out) {
	...
	// Clean up.
	if(out.Stype == superlu::SLU_NC) { superlu::destroy_compcol_mat(&out); }
	else if(out.Stype == superlu::SLU_NCP) { superlu::destroy_compcolperm_mat(&out); }
	...
	else if(out.Stype == superlu::SLU_SC) { superlu::destroy_supernode_mat(&out); }
	else {
		// Uh, crap.
...
(
...
#if defined(ARMA_USE_ARPACK)
	{
		...
		if((info != 0) && (info != 1)) {
			...
			if(sym) { arma_debug_warn_level(1, "eigs_sym(): ARPACK error ", info, " in saupd()"); }
			else { arma_debug_warn_level(1, "eigs_gen(): ARPACK error ", info, " in naupd()"); }
...
template<typename eT, typename T> inline
void sp_auxlib::run_aupd_shiftinvert
(
...
#if (defined(ARMA_USE_ARPACK) && defined(ARMA_USE_SUPERLU))
	{
		...
		if((info != 0) && (info != 1)) {
			...
			if(sym) { arma_debug_warn_level(2, "eigs_sym(): ARPACK error ", info, " in saupd()"); }
			else { arma_debug_warn_level(2, "eigs_gen(): ARPACK error ", info, " in naupd()"); }
...
inline
superlu::SuperMatrix& superlu_supermatrix_wrangler::get_ref() {
	used = true;
...
inline
superlu::SuperMatrix* superlu_supermatrix_wrangler::get_ptr() {
	used = true;
...
inline
superlu::SuperLUStat_t* superlu_stat_wrangler::get_ptr() { return &stat; }

//

template<typename eT> inline superlu_array_wrangler<eT>::~superlu_array_wrangler() {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> inline superlu_array_wrangler<eT>::superlu_array_wrangler(const uword n_elem) {
	arma_extra_debug_sigprint_this(this);
...
template<typename eT> inline
eT* superlu_array_wrangler<eT>::get_ptr() { return mem; }
