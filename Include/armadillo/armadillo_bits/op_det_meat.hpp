...
bool op_det::apply_direct(typename T1::elem_type& out_val, const Base<typename T1::elem_type, T1>& expr) {
...
template<typename T1> inline
typename T1::elem_type op_det::apply_diagmat(const Base<typename T1::elem_type, T1>& expr) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename T1::elem_type op_det::apply_trimat(const Base<typename T1::elem_type, T1>& expr) {
	arma_extra_debug_sigprint();
...
inline
eT op_det::apply_tiny(const Mat<eT>& X) {
	arma_extra_debug_sigprint();
	...
	if(N == 2) { return (Xm[pos<0, 0>::n2] * Xm[pos<1, 1>::n2] - Xm[pos<0, 1>::n2] * Xm[pos<1, 0>::n2]); }
