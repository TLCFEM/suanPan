...
namespace priv {
	...
	public:
		...
		typename arma_real_only<eT>::result nan(typename arma_real_only<eT>::result* junk = nullptr) {
			...
			if(std::numeric_limits<eT>::has_quiet_NaN) { return std::numeric_limits<eT>::quiet_NaN(); }
			else { return eT(0); }
		}
		...
		typename arma_real_only<eT>::result inf(typename arma_real_only<eT>::result* junk = nullptr) {
			...
			if(std::numeric_limits<eT>::has_infinity) { return std::numeric_limits<eT>::infinity(); }
			else { return std::numeric_limits<eT>::max(); }
		}
