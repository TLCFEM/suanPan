...
SpMat<eT>& SpMat<eT>::operator=(const SpSubview_col_list<eT, T1>& X) {
	...
	if(alias == false) { SpSubview_col_list<eT, T1>::extract(*this, X); }
	else {
		SpMat<eT> tmp(X);
...
void SpMat<eT>::shed_rows(const uword in_row1, const uword in_row2) {
	...
	while(it != it_end) {
		...
		if(lrow >= in_row1 && lrow <= in_row2) {
			...
			if(lrow > in_row2) { access::rw(newmat.row_indices[j]) = lrow - (in_row2 - in_row1 + 1); }
			else { access::rw(newmat.row_indices[j]) = lrow; }
...
void SpMat<eT>::set_size(const uword in_elem) {
	...
	if(vec_state == 2) { set_size(1, in_elem); }
	else { set_size(in_elem, 1); }
}
...
void SpMat<eT>::reshape(const uword in_rows, const uword in_cols) {
	...
	if(in_cols == 1) { (*this).reshape_helper_intovec(); }
	else { (*this).reshape_helper_generic(in_rows, in_cols); }
}
...
const SpMat<eT>& SpMat<eT>::replace(const eT old_val, const eT new_val) {
	...
	if(old_val == eT(0)) { arma_debug_warn_level(1, "SpMat::replace(): replacement not done, as old_val = 0"); }
	else {
		sync_csc();
		invalidate_cache();
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::save(const std::string name, const file_type type) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::save(const csv_name& spec, const file_type type) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::save(std::ostream& os, const file_type type) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::load(const std::string name, const file_type type) {
	arma_extra_debug_sigprint();
	...
	if(load_okay == false) {
		if(err_msg.length() > 0) { arma_debug_warn_level(3, "SpMat::load(): ", err_msg, "; file: ", name); }
		else { arma_debug_warn_level(3, "SpMat::load(): couldn't read; file: ", name); }
	}
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::load(const csv_name& spec, const file_type type) {
	arma_extra_debug_sigprint();
	...
	if(load_okay == false) {
		if(err_msg.length() > 0) { arma_debug_warn_level(3, "SpMat::load(): ", err_msg, "; file: ", spec.filename); }
		else { arma_debug_warn_level(3, "SpMat::load(): couldn't read; file: ", spec.filename); }
	}
	else {
		const uword load_n_cols = (do_trans) ? (*this).n_rows : (*this).n_cols;

		if(with_header && (spec.header_rw.n_elem != load_n_cols)) { arma_debug_warn_level(3, "SpMat::load(): size mistmach between header and matrix"); }
	}
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::load(std::istream& is, const file_type type) {
	arma_extra_debug_sigprint();
	...
	if(load_okay == false) {
		if(err_msg.length() > 0) { arma_debug_warn_level(3, "SpMat::load(): ", err_msg); }
		else { arma_debug_warn_level(3, "SpMat::load(): couldn't load from stream"); }
	}
...
void SpMat<eT>::init_batch_add(const Mat<uword>& locs, const Mat<eT>& vals, const bool sort_locations) {
	...
	if(sort_locations) {
		...
		if(actually_sorted == false) {
			...
			for(uword i = 1; i < sorted_indices.n_elem; ++i) {
				...
				if((locs_i[1] == locs_im1[1]) && (locs_i[0] == locs_im1[0])) { access::rw(values[count]) += vals[sorted_indices[i]]; }
				else {
					count++;
					access::rw(values[count]) = vals[sorted_indices[i]];
					access::rw(row_indices[count]) = locs_i[0];
	...
	if((sort_locations == false) || (actually_sorted == true)) {
		...
		for(uword i = 1; i < locs.n_cols; ++i) {
			...
			if((locs_i[1] == locs_im1[1]) && (locs_i[0] == locs_im1[0])) { access::rw(values[count]) += vals[i]; }
			else {
				count++;
...
void SpMat<eT>::steal_mem(SpMat<eT>& x) {
	...
	if((*this).vec_state == x.vec_state) { layout_ok = true; }
	else {
