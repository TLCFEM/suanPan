...
SP_STATUS Damper02::initialize(const shared_ptr<DomainBase>& D) {
    if(SUANPAN_SUCCESS != device->initialize_base(D) || SUANPAN_SUCCESS != device->initialize(D)) return SP_STATUS::FAIL;
