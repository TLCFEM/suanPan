...
SP_STATUS C3D8I::initialize(const shared_ptr<DomainBase>& D) {
    ...
    for(unsigned I{0}; I < plan.n_rows; ++I) {
        ...
        auto& c_pt = int_pt.emplace_back(std::move(t_vec), plan(I, 3) * det(jacob), mat_proto->unique_copy(), solve(jacob, pn));
