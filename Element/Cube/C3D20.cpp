...
std::vector<vec> C3D20::record(const OutputType P) const {
    std::vector<vec> data;
...
mat C3D20::GetDeformation(const double amplifier) { return get_coordinate(3).t() + amplifier * reshape(get_current_displacement(), c_dof, c_node); }
