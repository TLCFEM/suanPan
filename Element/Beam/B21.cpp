...
SP_STATUS B21::initialize(const shared_ptr<DomainBase>& D) {
    ...
    for(unsigned I{0}; I < int_pt_num; ++I) {
        ...
        local_stiffness += c_pt.strain_mat.t() * c_pt.b_section->get_initial_stiffness() * c_pt.strain_mat * c_pt.weight / length;
    }
...
std::vector<vec> B21::record(const OutputType P) const {
    std::vector<vec> data;
...
vtkSmartPointer<vtkCell> B21::GetCell() const { return vtkSmartPointer<vtkLine>::New(); }

mat B21::GetData(const OutputType P) {
