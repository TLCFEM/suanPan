...
SP_STATUS T2D2S::initialize(const shared_ptr<DomainBase>& D) {
...
vtkSmartPointer<vtkCell> T2D2S::GetCell() const { return vtkSmartPointer<vtkLine>::New(); }
...
mat T2D2S::GetData(const OutputType P) {
    ...
    vec data;
    if(const auto t_data = t_section->record(P); !t_data.empty()) data = t_data[0];
    return repmat(data.resize(6), 1, t_node);
}

mat T2D2S::GetDeformation(const double amplifier) { return get_coordinate(2).t() + amplifier * reshape(get_current_displacement(), t_dof, t_node); }
