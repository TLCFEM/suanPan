...
void new_csmt3(unique_ptr<Element>& return_obj, istringstream& command) {
	...
	auto thickness = 1.;
	if(!get_optional_input(command, thickness)) suanpan_debug("new_csmt3() assumes thickness to be unit.\n");

	auto length = -1.;
	if(!get_optional_input(command, length)) suanpan_debug("new_csmt3() assumes length to be area.\n");

	return_obj = make_unique<CSMT3>(tag, std::move(node_tag), material_tag, thickness, length);
}

void new_csmq4(unique_ptr<Element>& return_obj, istringstream& command) {
	unsigned tag;
	if(!get_input(command, tag)) {
		suanpan_error("new_csmq4() needs a tag.\n");
		return;
	}
	...
	for(auto& I : node_tag)
		if(!get_input(command, I)) {
			suanpan_error("new_csmq4() needs four valid nodes.\n");
			return;
		}

	unsigned material_tag;
	if(!get_input(command, material_tag)) {
		suanpan_error("new_csmq4() needs a valid material tag.\n");
		return;
	}

	auto thickness = 1.;
	if(!get_optional_input(command, thickness)) suanpan_debug("new_csmq4() assumes thickness to be unit.\n");
