...
int QE2::initialize(const shared_ptr<DomainBase>& D) {
    ...
    if(const auto t_density = mat_proto->get_density(); t_density > 0.) {
        ...
        for(const auto& I : int_pt) {
            ...
            for(auto J = 0u, P = 0u; J < m_node; ++J, P += m_dof)
                for(auto K = J, M = P; K < m_node; ++K, M += m_dof) initial_mass(P, M) += t_factor * n_int(J) * n_int(K);
        }
...
mat QE2::GetDeformation(const double amplifier) { return get_coordinate(2).t() + amplifier * get_current_displacement().reshape(m_dof, m_node).head_rows(2); }
