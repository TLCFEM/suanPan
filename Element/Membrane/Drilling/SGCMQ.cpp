...
int SGCMQ::initialize(const shared_ptr<DomainBase>& D) {
    ...
    for(unsigned I{0}; I < plan.n_rows; ++I) {
        ...
        auto& c_pt = int_pt.emplace_back(std::move(t_vec), det(jacob) * plan(I, 2) * thickness, mat_proto->unique_copy());
...
vtkSmartPointer<vtkCell> SGCMQ::GetCell() const { return vtkSmartPointer<vtkQuad>::New(); }
