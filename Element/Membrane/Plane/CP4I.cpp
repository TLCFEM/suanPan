...
int CP4I::initialize(const shared_ptr<DomainBase>& D) {
    ...
    for(unsigned I{0}; I < plan.n_rows; ++I) {
        ...
        auto& c_pt = int_pt.emplace_back(std::move(t_vec), plan(I, 2) * det(jacob) * thickness, material_proto->unique_copy(), solve(jacob, pn));
...
std::vector<vec> CP4I::record(const OutputType P) const {
    std::vector<vec> data;
...
mat CP4I::GetDeformation(const double amplifier) { return get_coordinate(2).t() + amplifier * reshape(get_current_displacement(), m_dof, m_node); }
