...
SP_STATUS CP4::initialize(const shared_ptr<DomainBase>& D) {
    ...
    const IntegrationPlan plan(2, reduced_scheme ? 1 : 2, IntegrationPlan::Type::GAUSS);
    ...
    for(auto I = 0u; I < plan.n_rows; ++I) {
        ...
        const auto& c_pt = int_pt.emplace_back(std::move(t_vec), plan(I, 2) * det(jacob), material_proto->unique_copy(), solve(jacob, pn));
...
std::vector<vec> CP4::record(const OutputType P) const {
    std::vector<vec> data;
...
vtkSmartPointer<vtkCell> CP4::GetCell() const { return vtkSmartPointer<vtkQuad>::New(); }
...
mat CP4::GetDeformation(const double amplifier) { return get_coordinate(2).t() + amplifier * reshape(get_current_displacement(), m_dof, m_node); }
