...
SP_STATUS CSMQ4::initialize(const shared_ptr<DomainBase>& D) {
...
std::vector<vec> CSMQ4::record(const OutputType P) const {
    std::vector<vec> data;
...
vtkSmartPointer<vtkCell> CSMQ4::GetCell() const { return vtkSmartPointer<vtkQuad>::New(); }
