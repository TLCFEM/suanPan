...
const uvec CSMQ4::t_dof{0, 1, 3, 4, 6, 7, 9, 10};
...
std::vector<vec> CSMQ4::record(const OutputType P) {
    std::vector<vec> data;
...
vtkSmartPointer<vtkCell> CSMQ4::Setup(const uvec& encoding) const {
...
void CSMQ4::GetData(vtkDoubleArray* const arrays, const OutputType type) {
...
void CSMQ4::SetDeformation(vtkPoints* const nodes, const double amplifier) {
