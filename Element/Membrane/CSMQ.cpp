...
void CSMQ::initialize(const shared_ptr<DomainBase>& D) {
	...
	for(unsigned I = 0; I < plan.n_rows; ++I) {
		...
		int_pt.emplace_back(std::move(t_vec), plan(I, 2) * det(jacob), material_proto->get_copy());
		...
		mat phi_s(2, 8, fill::zeros), l_p(3, 8, fill::zeros), j_p(1, 8, fill::zeros), j_q(2, 4, fill::zeros), phi_a = eye(3, 7);
		...
		phi_a(2, 6) = -(phi_a(1, 5) = phi_a(0, 4) = location(1));
		phi_a(2, 5) = -(phi_a(1, 3) = phi_a(0, 6) = location(0));

		const mat phi_b = solve(c_pt.m_material->get_initial_stiffness(), phi_a);

		const auto t_factor = c_pt.weight * thickness;

		E1 += t_factor * phi_r.t() * c_pt.m_material->get_initial_couple_stiffness() * phi_r;
		E2 += t_factor * phi_b.t() * c_pt.m_material->get_initial_stiffness() * phi_b;
