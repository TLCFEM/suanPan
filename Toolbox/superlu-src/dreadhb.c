...
static void FormFullA(int n, int_t* nonz, double** nzval, int_t** rowind, int_t** colptr) {
    ...
    if(!(t_val = (double*)SUPERLU_MALLOC(*nonz * sizeof(double)))) ABORT("SUPERLU_MALLOC fails for t_val[]");
    ...
    if(!(a_val = (double*)SUPERLU_MALLOC(new_nnz * sizeof(double)))) ABORT("SUPERLU_MALLOC fails for a_val[]");
