...
void zCreate_CompCol_Matrix(SuperMatrix* A, int m, int n, int nnz,
	...
	A->nrow = m;
	A->ncol = n;
	A->Store = SUPERLU_MALLOC(sizeof(NCformat));
...
void zCreate_CompRow_Matrix(SuperMatrix* A, int m, int n, int nnz,
	...
	A->nrow = m;
	A->ncol = n;
	A->Store = SUPERLU_MALLOC(sizeof(NRformat));
	if(!(A->Store)) ABORT("SUPERLU_MALLOC fails for A->Store");
	Astore = A->Store;
	Astore->nnz = nnz;
	Astore->nzval = nzval;
...
void zCreate_Dense_Matrix(SuperMatrix* X, int m, int n, doublecomplex* x, int ldx,
	...
	X->nrow = m;
	X->ncol = n;
	X->Store = SUPERLU_MALLOC(sizeof(DNformat));
...
void zCreate_SuperNode_Matrix(SuperMatrix* L, int m, int n, int nnz,
	...
	L->nrow = m;
	L->ncol = n;
	L->Store = SUPERLU_MALLOC(sizeof(SCformat));
