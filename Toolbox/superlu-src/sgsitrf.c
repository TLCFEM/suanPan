    ...
    *info = sLUMemInit(fact, work, lwork, m, n, Astore->nnz, panel_size, gamma, L, U, Glu, &iwork, &swork);
    ...
    for(jcol = 0; jcol < min_mn;) {
		...
		num_drop_L += i * (last - first + 1);
            ...
            if((*info = ilu_ssnode_dfs(jcol, kcol, asub, xa_begin, xa_end, marker, Glu)) != 0) return;
            ...
            for(jj = jcol; jj < jcol + panel_size; jj++) {
                ...
                if((*info = ilu_scolumn_dfs(m, jj, perm_r, &nseg, &panel_lsub[k], segrep, &repfnz[k], marker, parent, xplore, Glu))) return;
		    ...
		    fflush(stdout);
                ...
                if((*info = ilu_scopy_to_ucol(jj, nseg, segrep, &repfnz[k], perm_r, &dense[k], drop_rule, milu, amax[jj - jcol] * tol_U, quota, &drop_sum, &nnzUj, Glu, swork2)) != 0) return;
