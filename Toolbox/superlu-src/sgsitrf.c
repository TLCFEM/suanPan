...
void sgsitrf(superlu_options_t* options, SuperMatrix* A, int relax, int panel_size, int* etree, void* work, int_t lwork, int* perm_c, int* perm_r, SuperMatrix* L, SuperMatrix* U, GlobalLU_t* Glu, /* persistent to facilitate multiple factorizations */
             SuperLUStat_t* stat,
             int_t* info) {
    /* Local working arrays */
    NCPformat* Astore;
    ...
    *info = sLUMemInit(fact, work, lwork, m, n, Astore->nnz, panel_size, gamma, L, U, Glu, &iwork, &swork);
    if(*info) return;
    ...
    for(jcol = 0; jcol < min_mn;) {
        if(relax_end[jcol] != SLU_EMPTY) { /* start of a relaxed snode */
            ...
            if(jcol > 0 && jcol < last_drop) {
                ...
                /* Drop small rows */
                i = ilu_sdrop_row(options, first, last, tol_L, quota, &nnzLj, &fill_tol, Glu, tempv, swork2, 0);
            ...
            if((*info = ilu_ssnode_dfs(jcol, kcol, asub, xa_begin, xa_end, marker, Glu)) != 0)
                return;
            ...
            for(icol = jcol; icol <= kcol; icol++) {
                ...
                if((*info = ilu_spivotL(icol, diag_pivot_thresh, &usepr, perm_r, iperm_c[icol], swap, iswap, marker_relax, &pivrow, amax[0] * fill_tol, milu, zero, Glu, stat))) {
                    iinfo++;
                    marker[pivrow] = kcol;
                }
            }
            ...
            ilu_spanel_dfs(m, panel_size, jcol, A, perm_r, &nseg1, dense, amax, panel_lsub, segrep, repfnz, marker, parent, xplore, Glu);
            ...
            for(jj = jcol; jj < jcol + panel_size; jj++) {
                ...
                if((*info = ilu_scolumn_dfs(m, jj, perm_r, &nseg, &panel_lsub[k], segrep, &repfnz[k], marker, parent, xplore, Glu)))
                    return;
                ...
                if((*info = ilu_scopy_to_ucol(jj, nseg, segrep, &repfnz[k], perm_r, &dense[k], drop_rule, milu, amax[jj - jcol] * tol_U, quota, &drop_sum, &nnzUj, Glu, swork2)) != 0)
                    return;
                ...
                if(drop_sum != zero) {
                    if(drop_sum > zero)
                        omega = SUPERLU_MIN(2.0 * (1.0 - alpha) * amax[jj - jcol] / drop_sum, one);
                    else
                        omega = SUPERLU_MAX(2.0 * (1.0 - alpha) * amax[jj - jcol] / drop_sum, -one);
                    drop_sum *= omega;
                }
                if(usepr) pivrow = iperm_r[jj];
                fill_tol = pow(fill_ini, 1.0 - (double)jj / (double)min_mn);
                if((*info = ilu_spivotL(jj, diag_pivot_thresh, &usepr, perm_r, iperm_c[jj], swap, iswap, marker_relax, &pivrow, amax[jj - jcol] * fill_tol, milu, drop_sum, Glu, stat))) {
                    iinfo++;
                    marker[m + pivrow] = jj;
                ...
                if(jj > 0 && supno[jj] > supno[jj - 1] && jj < last_drop) {
                    ...
                    /* Drop small rows */
                    i = ilu_sdrop_row(options, first, last, tol_L, quota, &nnzLj, &fill_tol, Glu, tempv, swork2, 1);
