...
void sgstrs(trans_t trans, SuperMatrix* L, SuperMatrix* U, int* perm_c, int* perm_r, SuperMatrix* B, SuperLUStat_t* stat, int* info) {
    ...
    *info = 0;
    ...
    else if(L->nrow != L->ncol || L->nrow < 0 || L->Stype != SLU_SC || L->Dtype != SLU_S || L->Mtype != SLU_TRLU) *info = -2;
    else if(U->nrow != U->ncol || U->nrow < 0 || U->Stype != SLU_NC || U->Dtype != SLU_S || U->Mtype != SLU_TRU) *info = -3;
    else if(ldb < SUPERLU_MAX(0, L->nrow) || B->Stype != SLU_DN || B->Dtype != SLU_S || B->Mtype != SLU_GE) *info = -6;
    if(*info) {
        int ii = -(*info);
...
#endif
#else
                for(j = 0; j < nrhs; j++) susolve(nsupr, nsupc, &Lval[luptr], &Bmat[(size_t)fsupc + (size_t)j * (size_t)ldb]);
#endif
            }
