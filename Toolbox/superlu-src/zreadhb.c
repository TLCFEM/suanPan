 ...
 * </pre>
 */
static void FormFullA(int n, int_t* nonz, doublecomplex** nzval, int_t** rowind, int_t** colptr) {
    ...
    if(!(t_val = (doublecomplex*)SUPERLU_MALLOC(*nonz * sizeof(doublecomplex)))) ABORT("SUPERLU_MALLOC fails for t_val[]");
    ...
    if(!(a_val = (doublecomplex*)SUPERLU_MALLOC(new_nnz * sizeof(doublecomplex)))) ABORT("SUPERLU_MALLOC fails for a_val[]");
    ...
    for(j = 0; j < n; ++j) {
        for(i = t_colptr[j]; i < t_colptr[j + 1]; ++i) {
            if(t_rowind[i] != j) {
                ...
                a_val[k] = t_val[i];
                if(z_abs1(&a_val[k]) < 4.047e-300) printf("%5d: %e\t%e\n", (int)k, a_val[k].r, a_val[k].i);
                ++k;
            }
        }
...
void zreadhb(FILE* fp, int* nrow, int* ncol, int_t* nonz, doublecomplex** nzval, int_t** rowind, int_t** colptr) {
