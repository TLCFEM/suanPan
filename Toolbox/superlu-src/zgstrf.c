    ...
    *info = zLUMemInit(fact, work, lwork, m, n, Astore->nnz, panel_size, fill_ratio, L, U, Glu, &iwork, &zwork);
    ...
    for(jcol = 0; jcol < min_mn;) {
        if(relax_end[jcol] != EMPTY) {
            ...
            if((*info = zsnode_dfs(jcol, kcol, asub, xa_begin, xa_end, xprune, marker, Glu)) != 0) return;
            ...
            for(icol = jcol; icol <= kcol; icol++) {
                ...
                if((*info = zpivotL(icol, diag_pivot_thresh, &usepr, perm_r, iperm_r, iperm_c, &pivrow, Glu, stat))) if(iinfo == 0) iinfo = *info;
		...
		zprint_lu_col("[1]: ", icol, pivrow, xprune, Glu);
            ...
            for(jj = jcol; jj < jcol + panel_size; jj++) {
                ...
                if((*info = zcopy_to_ucol(jj, nseg, segrep, &repfnz[k], perm_r, &dense[k], Glu)) != 0) return;

                if((*info = zpivotL(jj, diag_pivot_thresh, &usepr, perm_r, iperm_r, iperm_c, &pivrow, Glu, stat))) if(iinfo == 0) iinfo = *info;
