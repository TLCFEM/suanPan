...
void sreadMM(FILE* fp, int* m, int* n, int_t* nonz, float** nzval, int_t** rowind, int_t** colptr) {
    ...
    *m = *n;
    ...
    if(!(val = (float*)SUPERLU_MALLOC(new_nonz * sizeof(double)))) ABORT("Malloc fails for val[]");
