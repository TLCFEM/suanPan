...
#define ITMAX 5
    ...
    *info = 0;
    ...
    else if(A->nrow != A->ncol || A->nrow < 0 || A->Stype != SLU_NC || A->Dtype != SLU_S || A->Mtype != SLU_GE) *info = -2;
    else if(L->nrow != L->ncol || L->nrow < 0 || L->Stype != SLU_SC || L->Dtype != SLU_S || L->Mtype != SLU_TRLU) *info = -3;
    else if(U->nrow != U->ncol || U->nrow < 0 || U->Stype != SLU_NC || U->Dtype != SLU_S || U->Mtype != SLU_TRU) *info = -4;
    else if(ldb < SUPERLU_MAX(0, A->nrow) || B->Stype != SLU_DN || B->Dtype != SLU_S || B->Mtype != SLU_GE) *info = -10;
    else if(ldx < SUPERLU_MAX(0, A->nrow) || X->Stype != SLU_DN || X->Dtype != SLU_S || X->Mtype != SLU_GE) *info = -11;
    if(*info != 0) {
        i = -(*info);
    ...
    for(j = 0; j < nrhs; ++j) {
		...
		SAXPY(&nrow, &done, work, &ione,
		       &Xmat[j*ldx], &ione);
#else
                saxpy_(&nrow, &done, work, &ione, &Xmat[j * ldx], &ione);
#endif
                lstres = berr[j];
                ++count;
            }
        ...
        do {
            slacon2_(&nrow, &work[A->nrow], work, &iwork[A->nrow], &ferr[j], &kase, isave);
            if(kase == 0) break;
