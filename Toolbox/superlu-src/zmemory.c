...
#define TempSpace(m, w) ((2 * w + 4 + NO_MARKER) * m * sizeof(int) + (w + 1) * m * sizeof(doublecomplex))
    ...
    if(fact != SamePattern_SameRowPerm) {
        ...
        while(!lusup || !ucol || !lsub || !usub) {
            if(Glu->MemModel == SYSTEM) {
                ...
                zuser_free((nzlumax + nzumax) * dword + (nzlmax + nzumax) * iword, HEAD, Glu);
            }
            nzlumax /= 2;
            nzumax /= 2;
            nzlmax /= 2;
 ...
 * </pre>
 */
int_t zLUMemXpand(int jcol, int_t next, /* number of elements currently in the factors */
