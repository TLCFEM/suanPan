...
int ilu_zQuerySpace(SuperMatrix* L, SuperMatrix* U, mem_usage_t* mem_usage) {
	...
	/* For LU factors */
	mem_usage->for_lu = (4.0f * n + 3.0f) * iword +
		Lstore->nzval_colptr[n] * dword +
		Lstore->rowind_colptr[n] * iword;
	mem_usage->for_lu += (n + 1.0f) * iword +
		Ustore->colptr[n] * (dword + iword);

	/* Working storage to support factorization.
	   ILU needs 5*n more integers than LU */
	mem_usage->total_needed = mem_usage->for_lu +
	((2.0f * panel_size + 9.0f + NO_MARKER) * n * iword +
...
int zLUMemInit(fact_t fact, void* work, int lwork, int m, int n, int annz,
	...
	fflush(stdout);
		...
		if(lwork == 0) { Glu->MemModel = SYSTEM; }
		else {
			Glu->MemModel = USER;
			Glu->stack.top2 = (lwork / 4) * 4; /* must be word-addressable */
			Glu->stack.size = Glu->stack.top2;
...
int zLUWorkInit(int m, int n, int panel_size, int** iworkptr,
	...
	if(Glu->MemModel == SYSTEM) *iworkptr = intCalloc(isize / sizeof(int));
...
* zexpand(
	...
	if(Glu->MemModel == SYSTEM) {
		new_mem = SUPERLU_MALLOC((size_t)new_len * lword);
