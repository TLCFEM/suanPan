    ...
    *info = 0;
    ...
    if(nrhs > 0) {
        ...
        if(notran) {
            if(rowequ) {
                for(j = 0; j < nrhs; ++j)
                    for(i = 0; i < n; ++i) zd_mult(&Bmat[i+j*ldb], &Bmat[i+j*ldb], R[i]);
            }
        }
        else if(colequ) {
            for(j = 0; j < nrhs; ++j)
                for(i = 0; i < n; ++i) { zd_mult(&Bmat[i+j*ldb], &Bmat[i+j*ldb], C[i]); }
        }
        ...
        if(notran) {
            if(colequ) {
                for(j = 0; j < nrhs; ++j)
                    for(i = 0; i < n; ++i) { zd_mult(&Xmat[i+j*ldx], &Xmat[i+j*ldx], C[i]); }
            }
        }
        else {
            /* transposed system */
