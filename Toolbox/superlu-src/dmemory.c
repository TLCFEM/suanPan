...
int_t dLUMemInit(fact_t fact, void* work, int_t lwork, int m, int n, int_t annz, int panel_size, double fill_ratio, SuperMatrix* L, SuperMatrix* U, GlobalLU_t* Glu, int** iwork, double** dwork) {
	...
	fflush(stdout);
        ...
        while(!lusup || !ucol || !lsub || !usub) {
            ...
            else { duser_free((nzlumax + nzumax) * dword + (nzlmax + nzumax) * iword, HEAD, Glu); }
            nzlumax /= 2;
            nzumax /= 2;
            nzlmax /= 2;
            if(nzlumax < annz) {
...
int dLUWorkInit(int m, int n, int panel_size, int** iworkptr, double** dworkptr, GlobalLU_t* Glu) {
    ...
    dsize = (m * panel_size + NUM_TEMPV(m, panel_size, maxsuper, rowblk)) * sizeof(double);
...
void* dexpand(
    ...
    if(Glu->MemModel == SYSTEM) {
        ...
        if(Glu->num_expansions != 0) {
            ...
            else { copy_mem_double(len_to_copy, expanders[type].mem, new_mem); }
