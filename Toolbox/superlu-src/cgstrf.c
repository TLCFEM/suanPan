...
void cgstrf(
    ...
    *info = cLUMemInit(fact, work, lwork, m, n, Astore->nnz, panel_size, fill_ratio, L, U, Glu, &iwork, &cwork);
    if(*info) return;
    ...
    for(jcol = 0; jcol < min_mn;) {
		...
		cprint_lu_col("[1]: ", icol, pivrow, xprune, Glu);
            ...
            cpanel_dfs(m, panel_size, jcol, A, perm_r, &nseg1, dense, panel_lsub, segrep, repfnz, xprune, marker, parent, xplore, Glu);
            ...
            for(jj = jcol; jj < jcol + panel_size; jj++) {
                ...
                if((*info = ccolumn_dfs(m, jj, perm_r, &nseg, &panel_lsub[k], segrep, &repfnz[k], xprune, marker, parent, xplore, Glu)) != 0) return;
