        ...
        if(strncmp(uplo, "L", 1) == 0) {
            ...
            for(k = 0; k <= Lstore->nsuper; k++) {
                ...
                if(nsupc == 1) {
                    ...
                    zgemv_("N", &nrow, &nsupc, &alpha, &Lval[luptr + nsupc], &nsupr, &x[fsupc], &incx, &beta, &work[0], &incy);
#endif
#else
                    zlsolve(nsupr, nsupc, &Lval[luptr], &x[fsupc]);

                    zmatvec(nsupr, nsupr - nsupc, nsupc, &Lval[luptr + nsupc], &x[fsupc], &work[0]);
#endif
...
#ifdef USE_VENDOR_BLAS
#ifdef _CRAY
                    CTRSV(ftcs3, ftcs2, ftcs2, &nsupc, &Lval[luptr], &nsupr, &x[fsupc], &incx);
#else
        ...
        if(strncmp(uplo, "L", 1) == 0) {
            ...
            for(k = Lstore->nsuper; k >= 0; --k) {
                ...
                if(nsupc > 1) {
                    ...
                    ftcs3 = _cptofcd("U", strlen("U"));
                    ZTRSV(ftcs1, ftcs2, ftcs3, &nsupc, &Lval[luptr], &nsupr, &x[fsupc], &incx);
#else
                    ztrsv_("L", trans, "U", &nsupc, &Lval[luptr], &nsupr, &x[fsupc], &incx);
#endif
                }
            }
        }
            ...
            for(k = 0; k <= Lstore->nsuper; k++) {
                ...
                if(nsupc == 1) {
                    ...
                    ZTRSV(ftcs1, ftcs2, ftcs3, &nsupc, &Lval[luptr], &nsupr, &x[fsupc], &incx);
#else
                    ztrsv_("U", trans, "N", &nsupc, &Lval[luptr], &nsupr, &x[fsupc], &incx);
#endif
                }
            } /* for k ... */
        }
