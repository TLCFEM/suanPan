...
void zgsitrf(superlu_options_t* options, SuperMatrix* A, int relax, int panel_size, int* etree, void* work, int_t lwork, int* perm_c, int* perm_r, SuperMatrix* L, SuperMatrix* U, GlobalLU_t* Glu, /* persistent to facilitate multiple factorizations */
    ...
    *info = zLUMemInit(fact, work, lwork, m, n, Astore->nnz, panel_size, gamma, L, U, Glu, &iwork, &zwork);
    if(*info) return;
    ...
    for(jcol = 0; jcol < min_mn;) {
        if(relax_end[jcol] != SLU_EMPTY) { /* start of a relaxed snode */
            ...
            for(icol = jcol; icol <= kcol; icol++) {
                ...
                if((*info = ilu_zpivotL(icol, diag_pivot_thresh, &usepr, perm_r, iperm_c[icol], swap, iswap, marker_relax, &pivrow, amax[0] * fill_tol, milu, zero, Glu, stat))) {
                    iinfo++;
                    marker[pivrow] = kcol;
                }
            }
            ...
            ilu_zpanel_dfs(m, panel_size, jcol, A, perm_r, &nseg1, dense, amax, panel_lsub, segrep, repfnz, marker, parent, xplore, Glu);
            ...
            for(jj = jcol; jj < jcol + panel_size; jj++) {
                ...
                if((*info = ilu_zpivotL(jj, diag_pivot_thresh, &usepr, perm_r, iperm_c[jj], swap, iswap, marker_relax, &pivrow, amax[jj - jcol] * fill_tol, milu, drop_sum, Glu, stat))) {
                    iinfo++;
                    marker[m + pivrow] = jj;
                ...
                if(jj > 0 && supno[jj] > supno[jj - 1] && jj < last_drop) {
                    ...
                    /* Drop small rows */
                    dtempv = (double*)tempv;
                    i = ilu_zdrop_row(options, first, last, tol_L, quota, &nnzLj, &fill_tol, Glu, dtempv, dwork2, 1);
