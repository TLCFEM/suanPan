...
#define TempSpace(m, w) ((2 * w + 4 + NO_MARKER) * m * sizeof(int) + (w + 1) * m * sizeof(float))
...
int_t sLUMemInit(fact_t fact, void* work, int_t lwork, int m, int n, int_t annz, int panel_size, float fill_ratio, SuperMatrix* L, SuperMatrix* U, GlobalLU_t* Glu, int** iwork, float** dwork) {
    ...
    if(fact != SamePattern_SameRowPerm) {
        ...
        if(lwork == -1) {
            return (GluIntArray(n) * iword + TempSpace(m, panel_size) + (nzlmax + nzumax) * iword + (nzlumax + nzumax) * dword + n);
        }
        else {
        ...
        while(!lusup || !ucol || !lsub || !usub) {
            if(Glu->MemModel == SYSTEM) {
                ...
                suser_free((nzlumax + nzumax) * dword + (nzlmax + nzumax) * iword, HEAD, Glu);
            }
            nzlumax /= 2;
            nzumax /= 2;
            nzlmax /= 2;
...
int sLUWorkInit(int m, int n, int panel_size, int** iworkptr, float** dworkptr, GlobalLU_t* Glu) {
    ...
    dsize = (m * panel_size +
             NUM_TEMPV(m, panel_size, maxsuper, rowblk)) *
            sizeof(float);
 ...
 * </pre>
 */
int_t sLUMemXpand(int jcol, int_t next, /* number of elements currently in the factors */
