...
void sgstrf(superlu_options_t* options, SuperMatrix* A, int relax, int panel_size, int* etree, void* work, int_t lwork, int* perm_c, int* perm_r, SuperMatrix* L, SuperMatrix* U, GlobalLU_t* Glu, /* persistent to facilitate multiple factorizations */
            SuperLUStat_t* stat,
            int_t* info) {
    /* Local working arrays */
    NCPformat* Astore;
    ...
    *info = sLUMemInit(fact, work, lwork, m, n, Astore->nnz, panel_size, fill_ratio, L, U, Glu, &iwork, &swork);
    ...
    for(jcol = 0; jcol < min_mn;) {
        if(relax_end[jcol] != SLU_EMPTY) { /* start of a relaxed snode */
            ...
            if((*info = ssnode_dfs(jcol, kcol, asub, xa_begin, xa_end, xprune, marker, Glu)) != 0)
                return;
            ...
            for(icol = jcol; icol <= kcol; icol++) {
                ...
                if((*info = spivotL(icol, diag_pivot_thresh, &usepr, perm_r, iperm_r, iperm_c, &pivrow, Glu, stat)))
            ...
            for(jj = jcol; jj < jcol + panel_size; jj++) {
                ...
                if((*info = scopy_to_ucol(jj, nseg, segrep, &repfnz[k], perm_r, &dense[k], Glu)) != 0)
                    return;

                if((*info = spivotL(jj, diag_pivot_thresh, &usepr, perm_r, iperm_r, iperm_c, &pivrow, Glu, stat)))
                    if(iinfo == 0) iinfo = *info;
