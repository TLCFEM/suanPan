...
int ilu_zpivotL(
    ...
    for(isub = nsupc; isub < nsupr; ++isub) {
        ...
        switch(milu) {
        case SMILU_1: z_add(&temp, &lu_col_ptr[isub], &drop_sum);
            rtemp = z_abs1(&temp);
            break;
        case SMILU_2:
        case SMILU_3:
	...
	fflush(stdout);
        ...
        if(*usepr) {
            switch(milu) {
            case SMILU_1: z_add(&temp, &lu_col_ptr[old_pivptr], &drop_sum);
                rtemp = z_abs1(&temp);
                break;
            case SMILU_2:
            case SMILU_3:
        ...
        if(*usepr == 0) {
            ...
            if(diag >= 0) {
                /* diagonal exists */
                switch(milu) {
                case SMILU_1: z_add(&temp, &lu_col_ptr[diag], &drop_sum);
                    rtemp = z_abs1(&temp);
                    break;
                case SMILU_2:
                case SMILU_3:
        ...
        /* Reset the diagonal */
        switch(milu) {
        case SMILU_1: z_add(&lu_col_ptr[pivptr], &lu_col_ptr[pivptr], &drop_sum);
            break;
        case SMILU_2:
        case SMILU_3:
    ...
    for(k = nsupc + 1; k < nsupr; k++) zz_mult(&lu_col_ptr[k], &lu_col_ptr[k], &temp);
