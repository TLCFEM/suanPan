        ...
        if(ksupno != jsupno) {
            ...
            if(kfnz != EMPTY) {
                ...
                while(new_next > nzumax) {
                    if((mem_error = zLUMemXpand(jcol, nextu, UCOL, &nzumax, Glu)) != 0) return (mem_error);
                    ucol = Glu->ucol;
                    if((mem_error = zLUMemXpand(jcol, nextu, USUB, &nzumax, Glu)) != 0) return (mem_error);
                    usub = Glu->usub;
                    lsub = Glu->lsub;
                }
                ...
                for(i = 0; i < segsze; i++) {
                    ...
                    if(quota > 0 && tmp >= drop_tol) {
                        ...
                        switch(milu) {
                        case SMILU_1:
                        case SMILU_2: z_add(sum, sum, &dense[irow]);
                            break;
                        case SMILU_3:
                            /* *sum += fabs(dense[irow]);*/
                            sum->r += tmp;
    ...
    if(drop_rule & DROP_SECONDARY && m > quota) {
        ...
        for(i = xusub[jcol]; i <= m0;) {
            if(z_abs1(&ucol[i]) <= tol) {
                switch(milu) {
                case SMILU_1:
                case SMILU_2: z_add(sum, sum, &ucol[i]);
                    break;
                case SMILU_3:
                    sum->r += tmp;
                    break;
