...
void cPrintPerf(SuperMatrix* L, SuperMatrix* U, mem_usage_t* mem_usage, float rpg, float rcond, float* ferr, const float* berr, const char* equed, SuperLUStat_t* stat) {
    SCformat* Lstore;
    NCformat* Ustore;
    double* utime;
    flops_t* ops;
...
int print_singlecomplex_vec(const char* what, int n, const singlecomplex* vec) {
    int i;
    printf("%s: n %d\n", what, n);
