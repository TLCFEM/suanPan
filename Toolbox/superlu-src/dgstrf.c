...
void dgstrf(
    ...
    *info = dLUMemInit(fact, work, lwork, m, n, Astore->nnz, panel_size, fill_ratio, L, U, Glu, &iwork, &dwork);
    if(*info) return;
    ...
    for(jcol = 0; jcol < min_mn;) {
        if(relax_end[jcol] != EMPTY) {
            ...
            if((*info = dsnode_dfs(jcol, kcol, asub, xa_begin, xa_end, xprune, marker, Glu)) != 0) return;
            ...
            for(icol = jcol; icol <= kcol; icol++) {
                ...
                if((*info = dpivotL(icol, diag_pivot_thresh, &usepr, perm_r, iperm_r, iperm_c, &pivrow, Glu, stat))) if(iinfo == 0) iinfo = *info;
		...
		dprint_lu_col("[1]: ", icol, pivrow, xprune, Glu);
            ...
            dpanel_dfs(m, panel_size, jcol, A, perm_r, &nseg1, dense, panel_lsub, segrep, repfnz, xprune, marker, parent, xplore, Glu);
            ...
            for(jj = jcol; jj < jcol + panel_size; jj++) {
                ...
                if((*info = dcolumn_dfs(m, jj, perm_r, &nseg, &panel_lsub[k], segrep, &repfnz[k], xprune, marker, parent, xplore, Glu)) != 0) return;

                /* Numeric updates */
                if((*info = dcolumn_bmod(jj, (nseg - nseg1), &dense[k], tempv, &segrep[nseg1], &repfnz[k], jcol, Glu, stat)) != 0) return;

                /* Copy the U-segments to ucol[*] */
                if((*info = dcopy_to_ucol(jj, nseg, segrep, &repfnz[k], perm_r, &dense[k], Glu)) != 0) return;

                if((*info = dpivotL(jj, diag_pivot_thresh, &usepr, perm_r, iperm_r, iperm_c, &pivrow, Glu, stat))) if(iinfo == 0) iinfo = *info;
