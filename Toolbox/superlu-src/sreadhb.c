 ...
 * </pre>
 */
static void FormFullA(int n, int_t* nonz, float** nzval, int_t** rowind, int_t** colptr) {
    ...
    if(!(t_val = (float*)SUPERLU_MALLOC(*nonz * sizeof(float)))) ABORT("SUPERLU_MALLOC fails for t_val[]");
    ...
    if(!(a_val = (float*)SUPERLU_MALLOC(new_nnz * sizeof(float)))) ABORT("SUPERLU_MALLOC fails for a_val[]");
