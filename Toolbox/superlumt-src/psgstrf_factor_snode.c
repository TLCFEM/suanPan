...
int_t psgstrf_factor_snode(
    ...
    if((*info = psgstrf_snode_dfs(pnum, jcol, kcol - 1, asub, xa_begin, xa_end, xprune, marker, col_lsub, pxgstrf_shared)))
        return 0;
    ...
    for(icol = jcol; icol < kcol; icol++) {
        ...
        psgstrf_snode_bmod(pnum, icol, jsupno, fsupc, dense, tempv, Glu, pxgstrf_shared->Gstat);

        if((*info = psgstrf_pivotL(pnum, icol, diag_pivot_thresh, usepr, perm_r, inv_perm_r, inv_perm_c, &pivrow, Glu, pxgstrf_shared->Gstat)))
