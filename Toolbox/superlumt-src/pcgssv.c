...
void pcgssv(int_t nprocs, SuperMatrix* A, int_t* perm_c, int_t* perm_r, SuperMatrix* L, SuperMatrix* U, SuperMatrix* B, int_t* info) {
    /*
    ...
    *info = 0;
    ...
    /* ------------------------------------------------------------
       ...
       ------------------------------------------------------------*/
    pcgstrf_init(nprocs, fact, trans, refact, panel_size, relax, diag_pivot_thresh, usepr, drop_tol, perm_c, perm_r, work, lwork, AA, &AC, &superlumt_options, &Gstat);
