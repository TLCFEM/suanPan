 ...
 */
    ...
    repfnz_col += m, dense_col += m) {
        ...
        if(segsze == 1) {
...
#else
            ctrsv_("L", "N", "U", &segsze, &lusup[luptr], &nsupr32, tempv, &incx);
#endif
...
#else
            cgemv_("N", &nrow32, &segsze, &alpha, &lusup[luptr], &nsupr32, tempv, &incx, &beta, tempv1, &incy);
#endif /* _CRAY_PVP */
#else
            ...
            for(i = 0; i < nrow; i++) {
                irow = lsub[isub];
                c_sub(&dense_col[irow], &dense_col[irow], &tempv1[i]); /* Scatter-add */
#ifdef SCATTER_FOUND
                if(col_marker[irow] != jj) {
                    col_marker[irow] = jj;
                    col_lsub[w_lsub_end[jj - jcol]++] = irow;
