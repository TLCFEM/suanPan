...
void pzgstrf(superlumt_options_t* superlumt_options, SuperMatrix* A, int_t* perm_r, SuperMatrix* L, SuperMatrix* U, Gstat_t* Gstat, int_t* info) {
    /*
    ...
    /* --------------------------------------------------------------
       ...
       --------------------------------------------------------------*/
    pzgstrf_threadarg = pzgstrf_thread_init(A, L, U, superlumt_options, &pxgstrf_shared, Gstat, info);
    if(*info) return;
...
#endif
        if((iinfo = thr_create(NULL, 0, pzgstrf_thread, &(pzgstrf_threadarg[i]), 0, &thread_id[i]))) {
    ...
    for(i = 0; i < nprocs; ++i) {
        if(iinfo = pthread_create(&thread_id[i], NULL, pzgstrf_thread, &(pzgstrf_threadarg[i]))) {
...
#pragma omp parallel for shared(pzgstrf_threadarg) private(i)
    ...
    /* ------------------------------------------------------------
       ...
       ------------------------------------------------------------*/
    pzgstrf_thread_finalize(pzgstrf_threadarg, &pxgstrf_shared, A, perm_r, L, U);
}
