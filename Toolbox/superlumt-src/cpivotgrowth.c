...
float cPivotGrowth(int_t ncols, SuperMatrix* A, int_t* perm_c,
	...
	for(k = 0; k <= Lstore->nsuper; ++k) {
		...
		for(j = fsupc; j < L_LAST_SUPC(k) && j < ncols; ++j) {
			maxaj = 0.;
			oldcol = inv_perm_c[j];
			for(i = Astore->colptr[oldcol]; i < Astore->colptr[oldcol + 1]; i++) maxaj = SUPERLU_MAX(maxaj, c_abs1( &Aval[i]));

			maxuj = 0.;
			for(i = Ustore->colbeg[j]; i < Ustore->colend[j]; i++) maxuj = SUPERLU_MAX(maxuj, c_abs1( &Uval[i]));

			/* Supernode */
			for(i = 0; i < nz_in_U; ++i) maxuj = SUPERLU_MAX(maxuj, c_abs1( &luval[i]));
