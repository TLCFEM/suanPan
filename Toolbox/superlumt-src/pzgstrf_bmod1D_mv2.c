...
void pzgstrf_bmod1D_mv2(
    ...
    repfnz_col += n, dense_col += n) {
        ...
        if(segsze == 1) {
            ...
            if(twocols == 1) {
...
#else
                    ztrsv_("L", "N", "U", &segsze, &lusup[luptr], &nsupr32, tri[j], &incx);
#endif
#else
...
#else
                    zgemv_("N", &nrow32, &segsze, &alpha, &lusup[luptr], &nsupr32, tri[0], &incx, &beta, matvec[0], &incy);
#endif
#else
                    zmatvec(nsupr, nrow, (int_t)segsze, &lusup[luptr], tri[0], matvec[0]);
#endif
                }
...
#else
                    zgemv_("N", &nrow32, &segsze, &alpha, &lusup[luptr], &nsupr32, tri[1], &incx, &beta, matvec[1], &incy);
#endif
#else
                    zmatvec(nsupr, nrow, (int_t)segsze, &lusup[luptr], tri[1], matvec[1]);
#endif
                }
                ...
                luptr = xlusup[fsupc] + nsupr * no_zeros + nsupc;
#if (MACH == DEC)
                zgemv2_(&nsupr, &nrow, &segsze, &lusup[luptr], &tri[0][kfnz - kfnz2[0]], &tri[1][kfnz - kfnz2[1]], matvec[0], matvec[1]);
                /*#elif ( MACH==CRAY_PVP )
                ZGEMV2(&nsupr, &nrow, &segsze, &lusup[luptr],
                       &tri[0][kfnz-kfnz2[0]], &tri[1][kfnz-kfnz2[1]],
                       matvec[0], matvec[1]);*/
#else
                zmatvec2(nsupr, nrow, (int_t)segsze, &lusup[luptr], &tri[0][kfnz - kfnz2[0]], &tri[1][kfnz - kfnz2[1]], matvec[0], matvec[1]);
#endif
                ...
                for(j = 0; j < 2; ++j) {
                    ...
                    for(i = 0; i < nrow; i++) {
                        irow = lsub[isub];
                        z_sub(&dense_col1[irow], &dense_col1[irow], &matvec[j][i]); /* Scatter-add */
#ifdef SCATTER_FOUND
    ...
    if(twocols == 1) { /* one more column left */
...
#else
        ztrsv_("L", "N", "U", &segsze, &lusup[luptr], &nsupr32, tri[0], &incx);
#endif
#else
        ...
        for(i = 0; i < nrow; i++) {
            irow = lsub[isub];
            z_sub(&dense_col1[irow], &dense_col1[irow], &matvec[0][i]); /* Scatter-add */
#ifdef SCATTER_FOUND
