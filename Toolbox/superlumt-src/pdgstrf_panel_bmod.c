...
void pdgstrf_panel_bmod(
    ...
    for(ksub = 0; ksub < *nseg; ++ksub) {
        ...
        if(nsupc >= colblk && nrow >= rowblk) {
            /* 2-D block update */
#ifdef GEMV2
            pdgstrf_bmod2D_mv2(pnum, m, w, jcol, fsupc, krep, nsupc, nsupr, nrow, repfnz, panel_lsub, w_lsub_end, spa_marker, dense, tempv, Glu, Gstat);
#else
            pdgstrf_bmod2D(pnum, m, w, jcol, fsupc, krep, nsupc, nsupr, nrow, repfnz, panel_lsub, w_lsub_end, spa_marker, dense, tempv, Glu, Gstat);
#endif
        }
        else {
            /* 1-D block update */
#ifdef GEMV2
            pdgstrf_bmod1D_mv2(pnum, m, w, jcol, fsupc, krep, nsupc, nsupr, nrow, repfnz, panel_lsub, w_lsub_end, spa_marker, dense, tempv, Glu, Gstat);
#else
            pdgstrf_bmod1D(pnum, m, w, jcol, fsupc, krep, nsupc, nsupr, nrow, repfnz, panel_lsub, w_lsub_end, spa_marker, dense, tempv, Glu, Gstat);
#endif
        }
    ...
    while(kcol < jcol) {
        ...
        kid = (pxgstrf_shared->pan_status[krep].size > 0) ?
                  krep :
                  (krep + pxgstrf_shared->pan_status[krep].size);
        desc_eft[ndesc].eft = cp_panel[kid].est + cp_panel[kid].pdiv;
        desc_eft[ndesc++].pmod = pmod;
#endif
    ...
    if(ndesc == 0) {
        ...
        for(j = cp_firstkid[jcol]; j != EMPTY; j = cp_nextkid[j]) {
            kid = (pxgstrf_shared->pan_status[j].size > 0) ?
                      j :
                      (j + pxgstrf_shared->pan_status[j].size);
            pmod_eft = SUPERLU_MAX(pmod_eft, cp_panel[kid].est + cp_panel[kid].pdiv);
        }
    }
