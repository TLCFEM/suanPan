    ...
    repfnz_col += n, dense_col += n) {
        ...
        if(segsze == 1) {
            ...
            if(twocols == 1) {
...
#else
                    dtrsv_("L", "N", "U", &segsze, &lusup[luptr], &nsupr32, tri[j], &incx);
#endif
#else
...
#else
                    dgemv_("N", &nrow32, &segsze, &alpha, &lusup[luptr], &nsupr32, tri[0], &incx, &beta, matvec[0], &incy);
#endif
#else
                    dmatvec(nsupr, nrow, (int_t)segsze, &lusup[luptr], tri[0], matvec[0]);
#endif
                }
...
#else
                    dgemv_("N", &nrow32, &segsze, &alpha, &lusup[luptr], &nsupr32, tri[1], &incx, &beta, matvec[1], &incy);
#endif
#else
                    dmatvec(nsupr, nrow, (int_t)segsze, &lusup[luptr], tri[1], matvec[1]);
#endif
                }
                ...
                luptr = xlusup[fsupc] + nsupr * no_zeros + nsupc;
#if (MACH == DEC)
                dgemv2_(&nsupr, &nrow, &segsze, &lusup[luptr], &tri[0][kfnz - kfnz2[0]], &tri[1][kfnz - kfnz2[1]], matvec[0], matvec[1]);
                /*#elif ( MACH==CRAY_PVP )
                DGEMV2(&nsupr, &nrow, &segsze, &lusup[luptr],
                       &tri[0][kfnz-kfnz2[0]], &tri[1][kfnz-kfnz2[1]],
                       matvec[0], matvec[1]);*/
#else
                dmatvec2(nsupr, nrow, (int_t)segsze, &lusup[luptr], &tri[0][kfnz - kfnz2[0]], &tri[1][kfnz - kfnz2[1]], matvec[0], matvec[1]);
#endif
...
#else
        dtrsv_("L", "N", "U", &segsze, &lusup[luptr], &nsupr32, tri[0], &incx);
#endif
#else
