 ...
 */
int_t superlu_zQuerySpace(int_t P, SuperMatrix* L, SuperMatrix* U, int_t panel_size, superlu_memusage_t* superlu_memusage) {
    SCPformat* Lstore;
    NCPformat* Ustore;
    ...
    superlu_memusage->for_lu = (float)(7 * n + 3) * iword + (float)Lstore->nzval_colend[n - 1] * dword + (float)Lstore->rowind_colend[n - 1] * iword;

    /* U columns of type NCP */
    superlu_memusage->for_lu += (2 * n + 1) * iword + (float)Ustore->colend[n - 1] * (dword + iword);
...
float pzgstrf_memory_use(const int_t nzlmax, const int_t nzumax, const int_t nzlumax) {
    ...
    t = 10. * ndim * iword + nzlmax * iword + nzumax * (iword + dword) + nzlumax * dword;
    return t;
}
 ...
 */
float pzgstrf_MemInit(int_t n, int_t annz, superlumt_options_t* superlumt_options, SuperMatrix* L, SuperMatrix* U, GlobalLU_t* Glu) {
    ...
    if(refact == NO) {
        ...
        if(lwork == -1) {
            return (GluIntArray(n) * iword + superlu_zTempSpace(n, panel_size, nprocs) + (nzlmax + nzumax) * iword + (nzlumax + nzumax) * dword);
        }
        else {
...
int_t pzgstrf_WorkInit(int_t n, int_t panel_size, int_t** iworkptr, doublecomplex** dworkptr) {
    ...
    dsize = (n * panel_size +
             NUM_TEMPV(n, panel_size, maxsuper, rowblk)) *
 ...
 */
void pzgstrf_SetRWork(int_t n, int_t panel_size, doublecomplex* dworkptr, doublecomplex** dense, doublecomplex** tempv) {
...
#pragma omp critical(STACK_LOCK)
            ...
            if(type != USUB) {
                ...
                bytes_to_copy = (char*)stack.array + stack.top1 - (char*)zexpanders[type + 1].mem;
