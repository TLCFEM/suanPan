 ...
 */
void pxgstrf_SetIWork(int_t n, int_t panel_size, int_t* iworkptr, int_t** segrep,
...
int_t Glu_alloc(
	...
	case LUSUP:
		/* Storage for the H-supernode is already set aside, so we do
		   not use lock here. */
		if(Glu->map_in_sup[jcol] < 0) fsupc = jcol + Glu->map_in_sup[jcol];
		else fsupc = jcol;
...
#pragma omp critical (ULOCK)
#endif
	{
		nextu = Glu->nextu;
		new_next = nextu + num;
		if(new_next > Glu->nzumax) { XPAND_HINT("U columns", new_next, jcol, 7); }
		*prev_next = nextu;
		Glu->nextu = new_next;
...
#pragma omp critical (LLOCK)
#endif
	{
		nextl = Glu->nextl;
		new_next = nextl + num;
		if(new_next > Glu->nzlmax) { XPAND_HINT("L subscripts", new_next, jcol, 8); }
		*prev_next = nextl;
		Glu->nextl = new_next;
...
#pragma omp critical (LULOCK)
#endif
	{
		...
		new_next = nextlu + num;
		if(new_next > Glu->nzlumax) { XPAND_HINT("L supernodes", new_next, jcol, 6); }
		Glu->nextlu = new_next;
	} /* end of critical region */
