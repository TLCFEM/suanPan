...
#ifdef USE_VENDOR_BLAS
#if (MACH == CRAY_PVP)
            CTRSV(ftcs1, ftcs2, ftcs3, &segsze, &lusup[luptr], &nsupr32, TriTmp, &incx);
#else
    ...
    for(r_ind = 0; r_ind < nrow; r_ind += rowblk) {
        ...
        for(jj = jcol; jj < jcol + w; ++jj, repfnz_col += n) {
            ...
            if(twocols == 1) { /* got two columns */
...
#ifdef USE_VENDOR_BLAS
#if (MACH == CRAY_PVP)
                    CGEMV(ftcs2, &block_nrow, &segsze, &alpha, &lusup[luptr], &nsupr32, tri[0], &incx, &beta, matvec[0], &incy);
#else
                    zgemv_("N", &block_nrow, &segsze, &alpha, &lusup[luptr], &nsupr32, tri[0], &incx, &beta, matvec[0], &incy);
#endif
#else
                    zmatvec((int_t)nsupr, (int_t)block_nrow, (int_t)segsze, &lusup[luptr], tri[0], matvec[0]);
#endif
                }
...
#ifdef USE_VENDOR_BLAS
#if (MACH == CRAY_PVP)
                    CGEMV(ftcs2, &block_nrow, &segsze, &alpha, &lusup[luptr], &nsupr, tri[1], &incx, &beta, matvec[1], &incy);
#else
                    zgemv_("N", &block_nrow, &segsze, &alpha, &lusup[luptr], &nsupr32, tri[1], &incx, &beta, matvec[1], &incy);
#endif
#else
                    zmatvec(nsupr, block_nrow, segsze, &lusup[luptr], tri[1], matvec[1]);
#endif
                }
                ...
                luptr = luptr1 + nsupr * no_zeros;
#if (MACH == DEC)
                zgemv2_(&nsupr, &block_nrow, &segsze, &lusup[luptr], &tri[0][kfnz - kfnz2[0]], &tri[1][kfnz - kfnz2[1]], matvec[0], matvec[1]);
                /*#elif ( MACH==CRAY_PVP )
                    ZGEMV2(&nsupr, &block_nrow, &segsze, &lusup[luptr],
                       &tri[0][kfnz-kfnz2[0]], &tri[1][kfnz-kfnz2[1]],
                       matvec[0], matvec[1]); */
#else
                zmatvec2(nsupr, block_nrow, segsze, &lusup[luptr], &tri[0][kfnz - kfnz2[0]], &tri[1][kfnz - kfnz2[1]], matvec[0], matvec[1]);
#endif
...
#ifdef USE_VENDOR_BLAS
#if (MACH == CRAY_PVP)
            CGEMV(ftcs2, &block_nrow, &segsze, &alpha, &lusup[luptr], &nsupr, tri[0], &incx, &beta, matvec[0], &incy);
#else
            zgemv_("N", &block_nrow, &segsze, &alpha, &lusup[luptr], &nsupr32, tri[0], &incx, &beta, matvec[0], &incy);
#endif
#else
            zmatvec(nsupr, block_nrow, segsze, &lusup[luptr], tri[0], matvec[0]);
#endif
        } /* if twocols == 1 */
        ...
        col_marker += n, col_lsub += n, matvec[0] += ldaTmp) {
            ...
            for(i = 0; i < block_nrow; ++i) {
                irow = lsub[isub];
                z_sub(&dense_col[irow], &dense_col[irow], &matvec[0][i]); /* Scatter-add */
#ifdef SCATTER_FOUND
                if(col_marker[irow] != jj) {
                    col_marker[irow] = jj;
                    col_lsub[w_lsub_end[jj - jcol]++] = irow;
