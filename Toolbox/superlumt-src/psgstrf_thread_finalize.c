...
void psgstrf_thread_finalize(psgstrf_threadarg_t* psgstrf_threadarg, pxgstrf_shared_t* pxgstrf_shared, SuperMatrix* A, int_t* perm_r, SuperMatrix* L, SuperMatrix* U) {
    /*
    ...
    if(superlumt_options->refact == YES) {
       ...
       pivoting, although the storage is available. */
        ...
        sCreate_SuperNode_Permuted(L, A->nrow, A->ncol, nnzL, Glu->lusup, Glu->xlusup, Glu->xlusup_end, Glu->lsub, Glu->xlsub, Glu->xlsub_end, Glu->supno, Glu->xsup, Glu->xsup_end, SLU_SCP, SLU_S, SLU_TRLU);
        sCreate_CompCol_Permuted(U, A->nrow, A->ncol, nnzU, Glu->ucol, Glu->usub, Glu->xusub, Glu->xusub_end, SLU_NCP, SLU_S, SLU_TRU);
    }
