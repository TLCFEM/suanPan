...
void zgsrfs(trans_t trans, SuperMatrix* A, SuperMatrix* L, SuperMatrix* U, int_t* perm_r, int_t* perm_c, equed_t equed, double* R, double* C, SuperMatrix* B, SuperMatrix* X, double* ferr, double* berr, Gstat_t* Gstat, int_t* info) {
    /*
...
#ifdef _CRAY
                CAXPY(&nrow, &done, work, &ione, &Xmat[j * ldx], &ione);
#else
