...
void cgstrs(trans_t trans, SuperMatrix* L, SuperMatrix* U, int_t* perm_r, int_t* perm_c, SuperMatrix* B, Gstat_t* Gstat, int_t* info) {
/*
        ...
        for(ksupno = 0; ksupno <= nsuper; ++ksupno) {
            ...
            if(nsupc == 1) {
...
#else
                ctrsm_("L", "L", "N", "U", &nsupc, &nrhs, &alpha, &Lval[luptr], &nsupr, &Bmat[fsupc], &ldb);

                cgemm_("N", "N", &nrow, &nrhs, &nsupc, &alpha, &Lval[luptr + nsupc], &nsupr, &Bmat[fsupc], &ldb, &beta, &work[0], &n);
#endif
                for(j = 0, bptr = 0; j < nrhs; j++, bptr += ldb) {
                    rhs_work = &Bmat[bptr];
                    work_col = &work[j * n];
                ...
                for(j = 0, bptr = 0; j < nrhs; j++, bptr += ldb) {
                    ...
                    cmatvec((int_t)nsupr, (int_t)nrow, (int_t)nsupc, &Lval[luptr + nsupc], &rhs_work[fsupc], &work[0]);
...
#else
                ctrsm_("L", "U", "N", "N", &nsupc, &nrhs, &alpha, &Lval[luptr], &nsupr, &Bmat[fsupc], &ldb);
#endif
#else
