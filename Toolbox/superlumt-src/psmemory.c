 ...
 */
int_t superlu_sQuerySpace(int_t P, SuperMatrix* L, SuperMatrix* U, int_t panel_size, superlu_memusage_t* superlu_memusage) {
    SCPformat* Lstore;
    NCPformat* Ustore;
 ...
 */
float psgstrf_MemInit(int_t n, int_t annz, superlumt_options_t* superlumt_options, SuperMatrix* L, SuperMatrix* U, GlobalLU_t* Glu) {
    ...
    if(refact == NO) {
        ...
        if(lwork == -1) {
            return (GluIntArray(n) * iword + superlu_sTempSpace(n, panel_size, nprocs) + (nzlmax + nzumax) * iword + (nzlumax + nzumax) * dword);
        }
        else {
 ...
 */
void psgstrf_SetRWork(int_t n, int_t panel_size, float* dworkptr, float** dense, float** tempv) {
    float zero = 0.0;
...
#pragma omp critical(STACK_LOCK)
            ...
            if(type != USUB) {
                ...
                bytes_to_copy = (char*)stack.array + stack.top1 - (char*)sexpanders[type + 1].mem;
