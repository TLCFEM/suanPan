...
typedef enum {
	ULOCK,
	/* locked once per column */
	LLOCK,
	/* locked once per supernode */
	LULOCK,
	/* locked once per column in L-supernode */
	NSUPER_LOCK,
	/* locked once per supernode */
	SCHED_LOCK,
	/* locked once per panel, if succeeded each time */
	STACK_LOCK,
	/* locked when upating Stack memory supplied by User */
	NO_GLU_LOCKS
} lu_locks_t;
