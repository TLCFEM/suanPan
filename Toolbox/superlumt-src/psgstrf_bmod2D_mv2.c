    ...
    for(r_ind = 0; r_ind < nrow; r_ind += rowblk) {
        ...
        for(jj = jcol; jj < jcol + w; ++jj, repfnz_col += n) {
            ...
            if(twocols == 1) { /* got two columns */
...
#else
                    sgemv_("N", &block_nrow, &segsze, &alpha, &lusup[luptr], &nsupr32, tri[0], &incx, &beta, matvec[0], &incy);
#endif
#else
                    smatvec((int_t)nsupr, (int_t)block_nrow, (int_t)segsze, &lusup[luptr], tri[0], matvec[0]);
#endif
                }
...
#else
                    sgemv_("N", &block_nrow, &segsze, &alpha, &lusup[luptr], &nsupr32, tri[1], &incx, &beta, matvec[1], &incy);
#endif
#else
                    smatvec(nsupr, block_nrow, segsze, &lusup[luptr], tri[1], matvec[1]);
#endif
                }
                ...
                luptr = luptr1 + nsupr * no_zeros;
#if (MACH == DEC)
                sgemv2_(&nsupr, &block_nrow, &segsze, &lusup[luptr], &tri[0][kfnz - kfnz2[0]], &tri[1][kfnz - kfnz2[1]], matvec[0], matvec[1]);
                /*#elif ( MACH==CRAY_PVP )
                    SGEMV2(&nsupr, &block_nrow, &segsze, &lusup[luptr],
                       &tri[0][kfnz-kfnz2[0]], &tri[1][kfnz-kfnz2[1]],
                       matvec[0], matvec[1]); */
#else
                smatvec2(nsupr, block_nrow, segsze, &lusup[luptr], &tri[0][kfnz - kfnz2[0]], &tri[1][kfnz - kfnz2[1]], matvec[0], matvec[1]);
#endif
...
#endif
#else
            smatvec(nsupr, block_nrow, segsze, &lusup[luptr], tri[0], matvec[0]);
#endif
        } /* if twocols == 1 */
