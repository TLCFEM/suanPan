 ...
 */
    ...
    for(r_ind = 0; r_ind < nrow; r_ind += rowblk) {
        ...
        repfnz_col += m, dense_col += m, TriTmp += ldaTmp) {
            ...
            beta = zero;
#if (MACH == CRAY_PVP)
            SGEMV(ftcs2, &block_nrow, &segsze, &alpha, &lusup[luptr], &nsupr32, TriTmp, &incx, &beta, MatvecTmp, &incy);
#else
            sgemv_("N", &block_nrow, &segsze, &alpha, &lusup[luptr1], &nsupr32, TriTmp, &incx, &beta, MatvecTmp, &incy);
#endif /* _CRAY_PVP */
#else
            smatvec(nsupr, (int_t)block_nrow, (int_t)segsze, &lusup[luptr1], TriTmp, MatvecTmp);
#endif
