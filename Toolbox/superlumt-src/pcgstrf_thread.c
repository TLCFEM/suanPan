    ...
    while(pxgstrf_shared->tasks_remain > 0) {
        ...
        if(jcol != EMPTY) {
...
#if (DEBUGlevel >= 3)
            printf("P%2d got panel %5d-%5d\ttime %.4f\tpanels_left %d\n", pnum, jcol, jcol + w - 1, SuperLU_timer_(), pxgstrf_shared->tasks_remain);
            fflush(stdout);
#endif
            /* Nondomain panels */
#ifdef PROFILE
            ...
            if(pxgstrf_shared->pan_status[jcol].type == RELAXED_SNODE) {
                ...
                pcgstrf_factor_snode(pnum, jcol, A, diag_pivot_thresh, usepr, perm_r, inv_perm_r, inv_perm_c, xprune, marker, panel_lsub, dense, tempv, pxgstrf_shared, info);
                if(*info) {
                    if(*info > n) return 0;
                ...
                TIC(t);
#endif
                pxgstrf_mark_busy_descends(pnum, jcol, etree, pxgstrf_shared, &bcol, lbusy);

                /* Symbolic factor on a panel of columns */
                pcgstrf_panel_dfs(pnum, m, w, jcol, A, perm_r, xprune, ispruned, lbusy, &nseg1, panel_lsub, w_lsub_end, segrep, repfnz, marker, spa_marker, parent, xplore, dense, Glu);
#if (DEBUGlevel >= 2)
                if(jcol == BADPAN)
                 ...
                 */
                pcgstrf_panel_bmod(pnum, m, w, jcol, bcol, inv_perm_r, etree, &nseg1, segrep, repfnz, panel_lsub, w_lsub_end, spa_marker, dense, tempv, pxgstrf_shared);
                ...
                for(jj = jcol; jj < jcol + w; jj++) {
                    ...
                    if(Glu->dynamic_snode_bound && super_bnd[jj]) {
                        ...
                        pxgstrf_super_bnd_dfs(pnum, m, n, jj, super_bnd[jj], A, perm_r, inv_perm_r, xprune, ispruned, marker1, parent, xplore, pxgstrf_shared);
                    }

                    if((*info = pcgstrf_column_dfs(pnum, m, jj, jcol, perm_r, ispruned, &panel_lsub[k], w_lsub_end[jj - jcol], super_bnd, &nseg, segrep, &repfnz[k], xprune, marker2, parent, xplore, pxgstrf_shared)))
                        return 0;
#ifdef PROFILE
                    TOC(t2, t);
                    utime[DFS] += t2;
                    ...
                    /* On return, the L supernode is gathered into the
                       global storage. */
                    if((*info = pcgstrf_column_bmod(pnum, jj, jcol, (nseg - nseg1), &segrep[nseg1], &repfnz[k], &dense[k], tempv, pxgstrf_shared, Gstat)))
                        return 0;

                    if((*info = pcgstrf_pivotL(pnum, jj, diag_pivot_thresh, usepr, perm_r, inv_perm_r, inv_perm_c, &pivrow, Glu, Gstat)))
                    ...
                    if((*info = pcgstrf_copy_to_ucol(pnum, jj, nseg, segrep, &repfnz[k], perm_r, &dense[k], pxgstrf_shared)))
                        return 0;

                    /* Prune columns [0:jj-1] using column jj */
                    pxgstrf_pruneL(jj, perm_r, pivrow, nseg, segrep, &repfnz[k], xprune, ispruned, Glu);
