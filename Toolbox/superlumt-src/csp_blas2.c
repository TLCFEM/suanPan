...
int_t sp_ctrsv(char* uplo, char* trans, char* diag, SuperMatrix* L, SuperMatrix* U, complex* x, int_t* info) {
/*
 *   Purpose
 *   =======
 *
        ...
        if(lsame_(uplo, "L")) {
            ...
            for(k = 0; k <= nsuper; k++) {
                ...
                if(nsupc == 1) {
                    ...
                    CTRSV(ftcs1, ftcs2, ftcs3, &nsupc, &Lval[luptr], &nsupr, &x[fsupc], &incx);

                    CGEMV(ftcs2, &nrow, &nsupc, &alpha, &Lval[luptr + nsupc], &nsupr, &x[fsupc], &incx, &beta, &work[0], &incy);
#else
                    ctrsv_("L", "N", "U", &nsupc, &Lval[luptr], &nsupr, &x[fsupc], &incx);

                    cgemv_("N", &nrow, &nsupc, &alpha, &Lval[luptr + nsupc], &nsupr, &x[fsupc], &incx, &beta, &work[0], &incy);
#endif
#else
                    clsolve(nsupr, nsupc, &Lval[luptr], &x[fsupc]);

                    cmatvec(nsupr, nsupr - nsupc, nsupc, &Lval[luptr + nsupc], &x[fsupc], &work[0]);
#endif
        ...
        if(lsame_(uplo, "L")) {
            ...
            for(k = Lstore->nsuper; k >= 0; --k) {
                ...
                if(nsupc > 1) {
                    ...
                    CTRSV(ftcs1, ftcs2, ftcs3, &nsupc, &Lval[luptr], &nsupr, &x[fsupc], &incx);
#else
                    ctrsv_("L", trans, "U", &nsupc, &Lval[luptr], &nsupr, &x[fsupc], &incx);
#endif
                }
            }
        }
            ...
            for(k = 0; k <= Lstore->nsuper; k++) {
                ...
                if(nsupc == 1) {
                    ...
                    CTRSV(ftcs1, ftcs2, ftcs3, &nsupc, &Lval[luptr], &nsupr, &x[fsupc], &incx);
#else
                    ctrsv_("U", trans, "N", &nsupc, &Lval[luptr], &nsupr, &x[fsupc], &incx);
#endif
                }
            } /* for k ... */
        }
...
int_t sp_cgemv(char* trans, complex alpha, SuperMatrix* A, complex* x, int_t incx, complex beta, complex* y, int_t incy) {
    /*  Purpose
        =======
