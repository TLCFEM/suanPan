...
fsupc %d, nsupr %d, nsupc %d\n",
            ...
            if(segsze == 1) {
...
#ifdef USE_VENDOR_BLAS
#if (MACH == CRAY_PVP)
                CTRSV(ftcs1, ftcs2, ftcs3, &segsze, &lusup[luptr], &nsupr, tempv, &incx);
#else
                ...
                beta = zero;
#if (MACH == CRAY_PVP)
                CGEMV(ftcs2, &nrow, &segsze, &alpha, &lusup[luptr], &nsupr, tempv, &incx, &beta, tempv1, &incy);
#else
                zgemv_("N", &nrow, &segsze, &alpha, &lusup[luptr], &nsupr, tempv, &incx, &beta, tempv1, &incy);
#endif
#else
    ...
    if((mem_error = Glu_alloc(pnum, jcol, nsupr, LUSUP, &nextlu, pxgstrf_shared)))
        return mem_error;
    xlusup[jcol] = nextlu;
    lusup = Glu->lusup;
    ...
    if(jcol == -1) {
        ...
        print_double_vec("before sup-col update", nrow, &lsub[xlsub[fsupc]], &lusup[xlusup[jcol]]);
    }
#endif
    ...
    if(fst_col < jcol) {
        ...
        zgemv_("N", &nrow, &nsupc, &alpha, &lusup[luptr + nsupc], &nsupr, &lusup[ufirst], &incx, &beta, &lusup[ufirst + nsupc], &incy);
#endif
#else
