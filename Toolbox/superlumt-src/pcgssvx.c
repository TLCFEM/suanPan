...
void pcgssvx(int_t nprocs, superlumt_options_t* superlumt_options, SuperMatrix* A, int_t* perm_c, int_t* perm_r, equed_t* equed, float* R, float* C, SuperMatrix* L, SuperMatrix* U, SuperMatrix* B, SuperMatrix* X, float* recip_pivot_growth, float* rcond, float* ferr, float* berr, superlu_memusage_t* superlu_memusage, int_t* info) {
    /*
    ...
    if(*info > 0) {
        ...
        *recip_pivot_growth = cPivotGrowth(A->ncol, AA, perm_c, L, U);
        ...
        /* ------------------------------------------------------------
           ...
           ------------------------------------------------------------*/
        t0 = SuperLU_timer_();
        cgsrfs(trant, AA, L, U, perm_r, perm_c, *equed, R, C, B, X, ferr, berr, &Gstat, info);
