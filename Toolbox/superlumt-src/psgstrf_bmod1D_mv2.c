        ...
        if(segsze == 1) {
            ...
            if(segsze == 2) {
                ...
                for(i = lptr + nsupc; i < xlsub_end[fsupc]; ++i) {
                    ...
                    ++luptr1;
                    ++luptr2;
                    dense_col[irow] -= (ukj * lusup[luptr] + ukj1 * lusup[luptr1] + ukj2 * lusup[luptr2]);
#ifdef SCATTER_FOUND
                    if(col_marker[irow] != jj) {
                        col_marker[irow] = jj;
                        col_lsub[w_lsub_end[jj - jcol]++] = irow;
            ...
            if(twocols == 1) {
...
#ifdef USE_VENDOR_BLAS
#if (MACH == CRAY_PVP)
                    STRSV(ftcs1, ftcs2, ftcs3, &segsze, &lusup[luptr], &nsupr32, tri[j], &incx);
#else
                    strsv_("L", "N", "U", &segsze, &lusup[luptr], &nsupr32, tri[j], &incx);
#endif
#else
...
#ifdef USE_VENDOR_BLAS
#if (MACH == CRAY_PVP)
                    SGEMV(ftcs2, &nrow32, &segsze, &alpha, &lusup[luptr], &nsupr32, tri[0], &incx, &beta, matvec[0], &incy);
#else
                    sgemv_("N", &nrow32, &segsze, &alpha, &lusup[luptr], &nsupr32, tri[0], &incx, &beta, matvec[0], &incy);
#endif
#else
                    smatvec(nsupr, nrow, (int_t)segsze, &lusup[luptr], tri[0], matvec[0]);
#endif
                }
...
#ifdef USE_VENDOR_BLAS
#if (MACH == CRAY_PVP)
                    SGEMV(ftcs2, &nrow32, &segsze, &alpha, &lusup[luptr], &nsupr, tri[1], &incx, &beta, matvec[1], &incy);
#else
                    sgemv_("N", &nrow32, &segsze, &alpha, &lusup[luptr], &nsupr32, tri[1], &incx, &beta, matvec[1], &incy);
#endif
#else
                    smatvec(nsupr, nrow, (int_t)segsze, &lusup[luptr], tri[1], matvec[1]);
#endif
                }
                ...
                luptr = xlusup[fsupc] + nsupr * no_zeros + nsupc;
#if (MACH == DEC)
                sgemv2_(&nsupr, &nrow, &segsze, &lusup[luptr], &tri[0][kfnz - kfnz2[0]], &tri[1][kfnz - kfnz2[1]], matvec[0], matvec[1]);
                /*#elif ( MACH==CRAY_PVP )
                SGEMV2(&nsupr, &nrow, &segsze, &lusup[luptr],
                       &tri[0][kfnz-kfnz2[0]], &tri[1][kfnz-kfnz2[1]],
                       matvec[0], matvec[1]);*/
#else
                smatvec2(nsupr, nrow, (int_t)segsze, &lusup[luptr], &tri[0][kfnz - kfnz2[0]], &tri[1][kfnz - kfnz2[1]], matvec[0], matvec[1]);
#endif
...
#ifdef USE_VENDOR_BLAS
#if (MACH == CRAY_PVP)
        STRSV(ftcs1, ftcs2, ftcs3, &segsze, &lusup[luptr], &nsupr32, tri[0], &incx);
#else
        strsv_("L", "N", "U", &segsze, &lusup[luptr], &nsupr32, tri[0], &incx);
#endif
#else
...
#ifdef USE_VENDOR_BLAS
#if (MACH == CRAY_PVP)
        SGEMV(ftcs2, &nrow, &segsze, &alpha, &lusup[luptr], &nsupr, tri[0], &incx, &beta, matvec[0], &incy);
#else
        sgemv_("N", &nrow32, &segsze, &alpha, &lusup[luptr], &nsupr32, tri[0], &incx, &beta, matvec[0], &incy);
#endif
#else
