...
mumps_c(MUMPS_STRUC_C *mumps_par)
{
  ...
  MUMPS_REAL *rinfo;
  MUMPS_REAL *rinfog;
  MUMPS_INT ooc_tmpdir[1023];
  MUMPS_INT ooc_prefix[255];
  MUMPS_INT write_problem[1023];
#if !defined(NO_SAVE_RESTORE)
  MUMPS_INT save_dir[1023];
  ...
  { /* job = -1: we just reset all pointers to 0 */
    ...
    mumps_par->colsca_loc = 0;
    mumps_par->rowsca_loc = 0;
    ...
    mumps_par->rhsintr = 0;
    mumps_par->glob2loc_rhs = 0;
    mumps_par->glob2loc_sol = 0;
    mumps_par->singular_values = 0;
...
#define EXTRACT_POINTERS(component, dummypointer) \
  ...
  MUMPS_F77(&(mumps_par->job), &(mumps_par->sym), &(mumps_par->par), &(mumps_par->comm_fortran),
            ...
            &isol_loc_avail, irhs_loc, &irhs_loc_avail, &(mumps_par->nz_rhs), &(mumps_par->lsol_loc), &(mumps_par->lrhs_loc), &(mumps_par->nsol_loc), &(mumps_par->nloc_rhs), &(mumps_par->schur_mloc), &(mumps_par->schur_nloc), &(mumps_par->schur_lld), &(mumps_par->mblock), &(mumps_par->nblock), &(mumps_par->nprow), &(mumps_par->npcol), &(mumps_par->ld_rhsintr), ooc_tmpdir, ooc_prefix, write_problem
#if !defined(NO_SAVE_RESTORE)
            ,
            save_dir, save_prefix
#endif
            ,
            &ooc_tmpdirlen, &ooc_prefixlen, &write_problemlen
#if !defined(NO_SAVE_RESTORE)
            ,
