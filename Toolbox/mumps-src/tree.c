...
fundamentalFronts(elimtree_t *T)
{
  ...
  while (J != -1)
  {
    ...
    while ((silbings[J] == -1) && (parent[J] != -1))
    {
      J = parent[J];
      child = firstchild[J];
      if ((silbings[child] != -1) || (ncolupdate[child] != ncolfactor[J] + ncolupdate[J]))
        frontmap[J] = cnfronts++;
      else
...
compressElimTree(elimtree_t *T, PORD_INT *frontmap, PORD_INT cnfronts)
{
  ...
  for (K = 0; K < nfronts; K++)
  {
    ...
    if (((pK = parent[K]) != -1) && ((pnewfront = frontmap[pK]) != newfront))
    {
...
justifyFronts(elimtree_t *T)
{
  ...
  for (K = firstPostorder(T); K != -1; K = nextPostorder(T, K))
  {
    ...
    else
    {
      ...
      while (nxtchild != -1)
      {
        s = s - minWspace[child] + ((ncolupdate[child] * (ncolupdate[child] + 1)) >> 1) + minWspace[nxtchild];
        m = max(m, s);
        child = nxtchild;
        nxtchild = silbings[nxtchild];
      }
      s = s - minWspace[child] + ((ncolupdate[child] * (ncolupdate[child] + 1)) >> 1) + frontsizeK;
      minWspace[K] = max(m, s);
    }
