...
graph_t* setupSubgraph(graph_t* G, PORD_INT* intvertex, PORD_INT nvint, PORD_INT* vtxmap) {
	...
	for(i = 0; i < nvint; i++) {
		...
		for(j = jstart; j < jstop; j++) {
			v = adjncy[j];
			if(vtxmap[v] >= 0) adjncyGsub[ptr++] = vtxmap[v];
		}
	}
	xadjGsub[nvint] = ptr;
	Gsub->type = G->type;
...
graph_t* setupGraphFromMtx(inputMtx_t* A) {
	...
	/* -----------------------------------------
	   ...
	   ----------------------------------------- */
	for(k = 0; k < neqs; k++) xadj[k] = xnza[k + 1] - xnza[k];
	for(k = 0; k < nelem; k++) xadj[nzasub[k]]++;
	...
	/* --------------------------------------------
	   ...
	   -------------------------------------------- */
	for(k = nvtx - 1; k > 0; k--) xadj[k] = xadj[k - 1];
	xadj[0] = 0;
	return (G);
}
...
graph_t* setupGridGraph(PORD_INT dimX, PORD_INT dimY, PORD_INT type) {
	...
	if((type == GRID) || (type == MESH)) {
		...
		if(type == MESH) nedges += 4 * (dimX - 1) * (dimY - 1); /* diagonals */
		...
		for(k = 0; k < nvtx; k++) {
			...
			if((k + 1) % dimX > 0) /*   / k+1-dimX  (MESH) */
			{
				...
				{
					if(k + 1 + dimX < nvtx) adjncy[knz++] = k + 1 + dimX;
					if(k + 1 - dimX >= 0) adjncy[knz++] = k + 1 - dimX;
				}
			}
...
static PORD_INT indNodes(graph_t* G, PORD_INT* vtxmap) {
	...
	for(u = 0; u < nvtx; u++) {
		...
		tmp[u] = -1;
		vtxmap[u] = u;
		for(i = istart; i < istop; i++) checksum[u] += adjncy[i];
	}
	...
	for(u = 0; u < nvtx; u++)
		if(vtxmap[u] == u) {
			...
			istart = xadj[u];
			istop = xadj[u + 1];
			for(i = istart; i < istop; i++) tmp[adjncy[i]] = u;
			...
			for(i = istart; i < istop; i++) {
				...
				if((v > u) && (checksum[v] == checksum[u]) && (deg[v] == deg[u])
					...
					jstart = xadj[v];
					jstop = xadj[v + 1];
					for(j = jstart; j < jstop; j++) if(tmp[adjncy[j]] != u) goto FAILURE;
...
graph_t* compressGraph(graph_t* G, PORD_INT* vtxmap) {
	...
	if((nvtxGc = indNodes(G, vtxmap)) > COMPRESS_FRACTION * nvtx) return (NULL);
	...
	for(i = 0; i < nedgesGc; i++) adjncyGc[i] = perm[adjncyGc[i]];
	for(u = 0; u < nvtx; u++) {
