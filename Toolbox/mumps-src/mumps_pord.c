...
(
	...
	*************************************/
	...
	for(u = nvtx; u >= 0; u--) { xadj_pe[u] = xadj_pe[u] - 1; }
	for(K = nedges - 1; K >= 0; K--) { adjncy[K] = adjncy[K] - 1; }
...
#          endif
		...
		if(parent[K] == -1) xadj_pe[vertex_root] = 0; /* root of the tree */
		else xadj_pe[vertex_root] = - (first[parent[K]] + 1);
...
(
	...
	*************************************/
	...
	mymalloc(G->vwght, nvtx, PORD_INT);
	for(u = 0; u < nvtx; u++) G->vwght[u] = nv[u];
	/* main function of the Ordering */
	T = SPACE_ordering(G, options, cpus);
	nfronts = T->nfronts;
	ncolfactor = T->ncolfactor;
