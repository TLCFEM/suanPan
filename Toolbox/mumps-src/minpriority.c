...
elimtree_t* orderMinPriority(minprior_t* minprior, options_t* options, timings_t* cpus) {
	...
	case MINIMUM_PRIORITY:
		break;
	case INCOMPLETE_ND:
		for(istage = 1; istage < nstages; istage++) eliminateStage(minprior, istage, scoretype, cpus);
		break;
	case MULTISECTION:
...
void eliminateStage(minprior_t* minprior, PORD_INT istage, PORD_INT scoretype, timings_t* cpus) {
  ...
  /* waitkey(); */
	...
	/* -------------------------------------
	   ...
	   ------------------------------------- */
	while(TRUE) {
		if(eliminateStep(minprior, istage, scoretype) == 0) break;
...
PORD_INT eliminateStep(minprior_t* minprior, PORD_INT istage, PORD_INT scoretype) {
  ...
  /* waitkey(); */
	...
	/* ----------------------
	   ...
	   ---------------------- */
	if((u = minBucket(bucket)) == -1) return (0);
	minscr = score[u];
	...
	while(TRUE) {
     ...
     printf("\n");
		...
		/* ---------------------------------------------------------------
		   ...
		   ---------------------------------------------------------------- */
		if(scoretype / 10 == 0) break;
		if((u = minBucket(bucket)) == -1) break;
		if(score[u] > minscr) break;
	}
