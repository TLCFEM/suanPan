  ...
  for (i = 0; i < nreach; i++)
  {
    ...
    if (vwght[u] > 0) /* u is a principal variable */
    {
      ...
      while (v != -1)
      {
        ...
        {
          keepon = TRUE;
          if ((len[w] != len[v]) || (elen[w] != elen[v]) || ((score[w] < 0) && (score[v] >= 0)) || ((score[w] >= 0) && (score[v] < 0)))
            keepon = FALSE;
          if (keepon)
          {
        ...
        if (bin[v] == 1) /* variables in Lme that have not */
        {
          ...
          if (deg > 40000 || degme > 40000)
          {
            ...
            case AMMF:
              ...
              break;
            case AMIND:
              scr_dbl = max(0, ((double)deg * (double)(deg - 1) / 2 - (double)degme * (double)(degme - 1) / 2) - (double)deg * (double)vwghtv);
              break;
            default:
          ...
          else
          {
            ...
            case AMMF:
              ...
              break;
            case AMIND:
              scr = max(0, (deg * (deg - 1) / 2 - degme * (degme - 1) / 2) - deg * vwghtv);
              break;
            default:
              fprintf(stderr, "\nError in function updateScore\n"
                              "  unrecognized selection strategy %d\n",
