...
void splitNDnode(nestdiss_t *nd, options_t *options, timings_t *cpus)
{
  ...
  for (i = 0; i < nvint; i++)
  {
    u = intvertex[i];
    if (intcolor[i] == BLACK)
      b_intvertex[b_nvint++] = u;
    if (intcolor[i] == WHITE)
      w_intvertex[w_nvint++] = u;
  }
  nd->childB = b_nd;
  b_nd->parent = nd;
  nd->childW = w_nd;
...
void buildNDtree(nestdiss_t *ndroot, options_t *options, timings_t *cpus)
{
  ...
  if (domainsize == 1)
    maxseps = DEFAULT_SEPS; /* secret switch */
  ...
  while ((qhead != qtail) && (seps < maxseps))
  {
    ...
    if (options[OPTION_MSGLVL] > 1)
      printf("%4d. S %6d, B %6d, W %6d [bal %4.2f, rel %6.4f, cost %7.2f]\n",
             ...
             (FLOAT)min(nd->cwght[BLACK], nd->cwght[WHITE]) / max(nd->cwght[BLACK], nd->cwght[WHITE]),
             (FLOAT)nd->cwght[GRAY] / (nd->cwght[GRAY] + nd->cwght[BLACK] + nd->cwght[WHITE]),
             F(nd->cwght[GRAY], nd->cwght[BLACK], nd->cwght[WHITE]));

    if ((nd->childB->nvint > MIN_NODES) && ((nd->cwght[BLACK] > domainsize) || (qtail < DEFAULT_SEPS)))
      queue[qtail++] = nd->childB;
    if ((nd->childW->nvint > MIN_NODES) && ((nd->cwght[WHITE] > domainsize) || (qtail < DEFAULT_SEPS)))
...
void freeNDtree(nestdiss_t *ndroot)
{
  ...
  {
    parent = nd->parent;
    if ((parent == NULL) || (parent->childB == NULL) || (parent->childW == NULL))
    {
