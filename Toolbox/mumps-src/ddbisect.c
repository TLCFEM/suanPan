               ...
               PORD_INT *tmp_color, PORD_INT *deltaW, PORD_INT *deltaB, PORD_INT *deltaS)
{
  ...
  for (i = istart; i < istop; i++)
  {
    ...
    if (deltaB[u] < 0)
      deltaB[u] = 1; /* the unique BLACK dom. flipped */
    deltaB[u]--;
    deltaW[u]++;
               ...
               PORD_INT *tmp_color, PORD_INT *deltaW, PORD_INT *deltaB, PORD_INT *deltaS)
{
  ...
  for (i = istart; i < istop; i++)
  {
    ...
    if (deltaW[u] < 0)
      deltaW[u] = 1; /* the unique WHITE dom. flipped */
    deltaB[u]++;
    deltaW[u]--;
...
INNER_LOOP_START:
  ...
  if (ftail != -1)
    ...
    fhead = -(domain + 1); /* list starts with domain */
