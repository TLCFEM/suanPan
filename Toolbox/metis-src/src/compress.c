...
graph_t* PruneGraph(ctrl_t* ctrl, idx_t nvtxs, idx_t* xadj, idx_t* adjncy, idx_t* vwgt, idx_t* iperm, real_t factor) {
    ...
    IFSET(ctrl->dbglvl, METIS_DBG_INFO, printf("  Pruned %"PRIDX" of %"PRIDX" vertices.\n", nlarge, nvtxs));
    ...
    if(nlarge > 0 && nlarge < nvtxs) {
        ...
        IFSET(ctrl->dbglvl, METIS_DBG_INFO, printf("  Pruning is ignored as it removes all vertices.\n"));
        nlarge = 0;
    }
