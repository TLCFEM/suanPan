...
void RefineKWay(ctrl_t* ctrl, graph_t* orggraph, graph_t* graph) {
    ...
    for (i = 0;; i++) {
        ...
        IFSET(ctrl->dbglvl, METIS_DBG_TIME, gk_startcputimer(ctrl->ProjectTmr));ASSERT(graph->vwgt != NULL);
    ...
    if(ctrl->contig)ASSERT(FindPartitionInducedComponents(graph, graph->where, NULL, NULL) == ctrl->nparts);
...
void AllocateKWayPartitionMemory(ctrl_t* ctrl, graph_t* graph) {
    ...
    case METIS_OBJTYPE_CUT:
        graph->ckrinfo =
            (ckrinfo_t*) gk_malloc(graph->nvtxs * sizeof(ckrinfo_t), "AllocateKWayPartitionMemory: ckrinfo");
        break;

    case METIS_OBJTYPE_VOL:
        graph->vkrinfo =
            (vkrinfo_t*) gk_malloc(graph->nvtxs * sizeof(vkrinfo_t), "AllocateKWayVolPartitionMemory: vkrinfo");
...
void ComputeKWayPartitionParams(ctrl_t* ctrl, graph_t* graph) {
    ...
    /* Compute pwgts */
    if(ncon == 1) {
        for (i = 0; i < nvtxs; i++) { ASSERT(where[i] >= 0 && where[i] < nparts);
