...
graph_t* CoarsenGraph(ctrl_t* ctrl, graph_t* graph) {
    ...
    } while (graph->nvtxs > ctrl->CoarsenTo && graph->nvtxs < COARSEN_FRACTION * graph->finer->nvtxs
...
graph_t* CoarsenGraphNlevels(ctrl_t* ctrl, graph_t* graph, idx_t nlevels) {
    ...
    for (level = 0; level < nlevels; level++) {
        ...
        if(graph->nvtxs < ctrl->CoarsenTo || graph->nvtxs > COARSEN_FRACTION * graph->finer->nvtxs
            || graph->nedges < graph->nvtxs / 2)
            break;
    }
...
idx_t Match_SHEM(ctrl_t* ctrl, graph_t* graph) {
    idx_t i, pi, ii, j, jj, jjinc, k, nvtxs, ncon, cnvtxs, maxidx, maxwgt, last_unmatched, avgdegree;
    idx_t* xadj, * vwgt, * adjncy, * adjwgt, * maxvwgt;
            ...
            if((ncon == 1 ? vwgt[i] < maxvwgt[0] : ivecle(ncon, vwgt + i * ncon, maxvwgt))) {
                ...
                if(xadj[i] == xadj[i + 1]) {
                    ...
                    if(ncon == 1) {
                        ...
                        for (j = xadj[i]; j < xadj[i + 1]; j++) {
                            k = adjncy[j];
                            if(match[k] == UNMATCHED && ivecaxpylez(ncon, 1, vwgt + i * ncon, vwgt + k * ncon, maxvwgt)
                                && (maxwgt < adjwgt[j] || (maxwgt == adjwgt[j] && BetterVBalance(ncon,
                                                                                                 graph->invtvwgt,
                                                                                                 vwgt + i * ncon,
                                                                                                 vwgt + maxidx * ncon,
                                                                                                 vwgt + k * ncon)))) {
...
void CreateCoarseGraph(ctrl_t* ctrl, graph_t* graph, idx_t cnvtxs, idx_t* match) {
    idx_t j, jj, k, kk, l, m, istart, iend, nvtxs, nedges, ncon, cnedges, v, u, mask, dovsize;
    ...
    for (v = 0; v < nvtxs; v++) {
        ...
        ASSERT(cmap[v] == cnvtxs);ASSERT(cmap[match[v]] == cnvtxs);
...
void CreateCoarseGraphPerm(ctrl_t* ctrl, graph_t* graph, idx_t cnvtxs, idx_t* match, idx_t* perm) {
    idx_t i, j, jj, k, kk, l, m, istart, iend, nvtxs, nedges, ncon, cnedges, v, u, mask, dovsize;
    idx_t* xadj, * vwgt, * vsize, * adjncy, * adjwgt;
    idx_t* cmap, * htable;
    idx_t* cxadj, * cvwgt, * cvsize, * cadjncy, * cadjwgt;
    graph_t* cgraph;
