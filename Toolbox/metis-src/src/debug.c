...
idx_t CheckBnd2(graph_t* graph) {
    ...
    for (nbnd = 0, i = 0; i < nvtxs; i++) {
        ...
        if(ed - id >= 0 && xadj[i] < xadj[i + 1]) {
            nbnd++;ASSERTP(bndptr[i] != -1, ("%"PRIDX" %"PRIDX" %"PRIDX"\n", i, id, ed));ASSERT(bndind[bndptr[i]] == i);
        }
    }
...
idx_t CheckNodeBnd(graph_t* graph, idx_t onbnd) {
    ...
    for (i = 0; i < nvtxs; i++) {
        if(where[i] != 2) { ASSERTP(bndptr[i] == -1, ("%"PRIDX" %"PRIDX"\n", i, bndptr[i]));
        }
        else { ASSERTP(bndptr[i] != -1, ("%"PRIDX" %"PRIDX"\n", i, bndptr[i]));
        }
    }
...
idx_t CheckRInfo(ctrl_t* ctrl, ckrinfo_t* rinfo) {
    ...
    for (i = 0; i < rinfo->nnbrs; i++) {
        for (j = i + 1; j < rinfo->nnbrs; j++)ASSERTP(nbrs[i].pid != nbrs[j].pid,
