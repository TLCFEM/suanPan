    ...
    for (nswaps = 0; nswaps < nvtxs; nswaps++) {
        ...
        IFSET(ctrl->dbglvl,
              ...
              printf("Moved %6"PRIDX" from %"PRIDX". [%3"PRIDX" %3"PRIDX"] %5"PRIDX" [%4"PRIDX" %4"PRIDX"]\n", higain, from,
                  ed[higain] - id[higain], vwgt[higain], mincut, pwgts[0], pwgts[1]));
...
void McGeneral2WayBalance(ctrl_t* ctrl, graph_t* graph, real_t* ntpwgts) {
    ...
    minbal = ComputeLoadImbalanceDiffVec(graph, 2, ctrl->pijbm, ctrl->ubfactors, minbalv);ASSERT(minbal > 0.0);
    ...
    for (nswaps = 0; nswaps < nvtxs; nswaps++) {
        ...
        if(newbal < minbal || (newbal == minbal
            && (newcut < mincut || (newcut == mincut && BetterBalance2Way(ncon, minbalv, newbalv))))) {
            mincut = newcut;
            minbal = newbal;
            mincutorder = nswaps;
    ...
    *****************************************************************/
    ...
    if(ctrl->dbglvl & METIS_DBG_REFINE) {
        printf("\tMincut: %6"PRIDX" at %5"PRIDX", NBND: %6"PRIDX", NPwgts: [", mincut, mincutorder, nbnd);
        for (l = 0; l < ncon; l++)
            printf("(%6"PRIDX", %6"PRIDX") ", pwgts[l], pwgts[ncon + l]);
