...
void ComputeSubDomainGraph(ctrl_t* ctrl, graph_t* graph) {
    ...
    for (pid = 0; pid < nparts; pid++) {
        ...
        if(ctrl->maxnads[pid] < nads) {
            ...
            ctrl->adids[pid] = irealloc(ctrl->adids[pid], ctrl->maxnads[pid], "ComputeSubDomainGraph: adids[pid]");
            ctrl->adwgts[pid] = irealloc(ctrl->adwgts[pid], ctrl->maxnads[pid], "ComputeSubDomainGraph: adids[pid]");
        }
    ...
    for (i = 0; i < 2; i++) {
        ...
        if(j == nads) {
            ...
            if(ctrl->maxnads[u] == nads) {
                ...
                ctrl->adids[u] = irealloc(ctrl->adids[u], ctrl->maxnads[u], "IncreaseEdgeSubDomainGraph: adids[pid]");
                ctrl->adwgts[u] = irealloc(ctrl->adwgts[u], ctrl->maxnads[u], "IncreaseEdgeSubDomainGraph: adids[pid]");
            }
            ctrl->adids[u][nads] = v;
...
void EliminateSubDomainEdges(ctrl_t* ctrl, graph_t* graph) {
    idx_t i, ii, j, k, ncon, nparts, scheme, pid_from, pid_to, me, other, nvtxs, total, max, avg, totalout, nind = 0,
        ncand = 0, ncand2, target, target2, nadd, bestnadd = 0;
    idx_t min, move, * cpwgt;
    idx_t* xadj, * adjncy, * vwgt, * adjwgt, * pwgts, * where, * maxpwgt, * mypmat, * otherpmat, * kpmat, * ind;
    ...
    for (i = 0; i < nparts; i++) {
        for (j = 0; j < ncon; j++)
            maxpwgt[i * ncon + j] = (ncon == 1 ? 1.25 : 1.025) * tpwgts[i] * graph->tvwgt[j] * ctrl->ubfactors[j];
    }
    ...
    while (1) {
        ...
        IFSET(ctrl->dbglvl, METIS_DBG_CONNINFO, printf("Adjacent Subdomain Stats: Total: %3"PRIDX", "
        ...
        while ((me = ipqGetTop(&queue)) != -1) {
            ...
            for (scheme = 0; scheme < 2; scheme++) {
                for (min = 0; min < ncand2; min++) {
                    ...
                    for (nind = 0, ii = pptr[pid_from]; ii < pptr[pid_from + 1]; ii++) {
                        i = pind[ii];ASSERT(where[i] == pid_from);
                    ...
                    for (i = 0; i < ncand; i++) {
                        cand[i].key = otherpmat[cand[i].val];ASSERT(cand[i].key > 0);
                    }
            ...
            if(target != -1) {
                ...
                case METIS_OBJTYPE_CUT:MoveGroupMinConnForCut(ctrl, graph, target, nind, ind);
                    break;
                case METIS_OBJTYPE_VOL: MoveGroupMinConnForVol(ctrl,
                                                               graph,
                                                               target,
                                                               nind,
                                                               ind,
...
void MoveGroupMinConnForCut(ctrl_t* ctrl, graph_t* graph, idx_t to, idx_t nind, idx_t* ind) {
    ...
    while (--nind >= 0) {
        ...
        if(k == myrinfo->nnbrs) { ASSERT(k < xadj[i + 1] - xadj[i]);
            mynbrs[k].pid = to;
            mynbrs[k].ed = 0;
            myrinfo->nnbrs++;
        }
        ...
        UpdateMovedVertexInfoAndBND(i, from, k, to, myrinfo, mynbrs, where, nbnd, bndptr, bndind, BNDTYPE_REFINE);
...
void MoveGroupMinConnForVol(ctrl_t* ctrl,
    ...
    while (--nind >= 0) {
        ...
        if(k == myrinfo->nnbrs) {
            ...
            for (j = xadj[i]; j < xadj[i + 1]; j++) {
                ...
                onbrs = ctrl->vnbrpool + orinfo->inbr;ASSERT(other != to)
    ...
    }ASSERT(ComputeCut(graph, where) == graph->mincut);ASSERTP(ComputeVolume(graph, where) == graph->minvol,
                                                               ("%"PRIDX" %"PRIDX"\n", ComputeVolume(graph,
                                                                                                     where), graph->minvol));
