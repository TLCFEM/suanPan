...
void Greedy_KWayCutOptimize(ctrl_t* ctrl, graph_t* graph, idx_t niter, real_t ffactor, idx_t omode) {
  ...
  *======================================================================*/
    for (pass = 0; pass < niter; pass++) { ASSERT(ComputeCut(graph, where) == graph->mincut);
        ...
        for (ii = 0; ii < nbnd; ii++) {
            i = bndind[perm[ii]];
            rgain = (graph->ckrinfo[i].nnbrs > 0 ? 1.0 * graph->ckrinfo[i].ed / sqrt(graph->ckrinfo[i].nnbrs) : 0.0)
                - graph->ckrinfo[i].id;
        ...
        for (nmoved = 0, iii = 0;; iii++) {
            ...
            if(omode == OMODE_REFINE) {
                ...
                gain = mynbrs[k].ed - myrinfo->id;
                if(!(gain > 0 || (gain == 0
                    && (pwgts[from] >= maxwgt[from] || itpwgts[to] * pwgts[from] > itpwgts[from] * (pwgts[to] + vwgt)
                ...
                if(pwgts[from] < maxwgt[from] && pwgts[to] > minwgt[to] && mynbrs[k].ed - myrinfo->id < 0)
                    continue;
            }
      ...
      *======================================================================*/
            ...
            UpdateMovedVertexInfoAndBND(i, from, k, to, myrinfo, mynbrs, where, nbnd, bndptr, bndind, bndtype);
            ...
            for (j = xadj[i]; j < xadj[i + 1]; j++) {
                ...
                UpdateAdjacentVertexInfoAndBND(ctrl,
                                               ii,
                                               xadj[ii + 1] - xadj[ii],
                                               me,
                                               from,
                ...
                UpdateQueueInfo(queue, vstatus, ii, me, from, to, myrinfo, oldnnbrs, nupd, updptr, updind, bndtype);
        ...
        for (i = 0; i < nupd; i++) { ASSERT(updptr[updind[i]] != -1);ASSERT(vstatus[updind[i]] != VPQSTATUS_NOTPRESENT);
...
void Greedy_KWayVolOptimize(ctrl_t* ctrl, graph_t* graph, idx_t niter, real_t ffactor, idx_t omode) {
  ...
  * The top-level refinement loop
  *======================================================================*/
    for (pass = 0; pass < niter; pass++) { ASSERT(ComputeVolume(graph, where) == graph->minvol);
        ...
        for (nmoved = 0, iii = 0;; iii++) {
            ...
            if(omode == OMODE_REFINE) {
                ...
                for (j = k - 1; j >= 0; j--) {
                    ...
                    if((mynbrs[j].gv > mynbrs[k].gv && pwgts[to] + vwgt <= maxwgt[to] + ffactor * gain)
                ...
                if(xgain + mynbrs[k].gv > 0 || mynbrs[k].ned - myrinfo->nid > 0)
                    ...
                    if((iii % 2 == 0 && safetos[to] == 2) || pwgts[from] >= maxwgt[from]
                        || itpwgts[from] * (pwgts[to] + vwgt) < itpwgts[to] * pwgts[from])
                        j = 1;
                }
                if(j == 0)
                ...
                if(pwgts[from] < maxwgt[from] && pwgts[to] > minwgt[to]
                    && (xgain + mynbrs[k].gv < 0 || (xgain + mynbrs[k].gv == 0 && mynbrs[k].ned - myrinfo->nid < 0)))
                    continue;
            }
      ...
      *======================================================================*/
            ...
            IFSET(ctrl->dbglvl, METIS_DBG_MOVEINFO, printf("\t\tMoving %6"PRIDX" from %3"PRIDX" to %3"PRIDX". "
                                                                                                           "Gain: [%4"PRIDX" %4"PRIDX"]. Cut: %6"PRIDX", Vol: %6"PRIDX"\n", i, from, to,
                xgain + mynbrs[k].gv, mynbrs[k].ned - myrinfo->nid, graph->mincut, graph->minvol));
        ...
        if(nmoved == 0 || (omode == OMODE_REFINE && graph->minvol == oldvol && graph->mincut == oldcut))
            break;
    }
            ...
            if(omode == OMODE_REFINE) {
                ...
                for (j = k - 1; j >= 0; j--) {
                    ...
                    if((mynbrs[j].ed > mynbrs[k].ed
                        ...
                        || (mynbrs[j].ed == mynbrs[k].ed && BetterBalanceKWay(ncon,
                                                                              vwgt + i * ncon,
                                                                              ubfactors,
                                                                              1,
                                                                              pwgts + cto * ncon,
                ...
                gain = mynbrs[k].ed - myrinfo->id;
                if(!(gain > 0 || (gain == 0 && (BetterBalanceKWay(ncon,
                                                                  vwgt + i * ncon,
                                                                  ubfactors,
                                                                  -1,
                                                                  pwgts + from * ncon,
                ...
                if(mynbrs[k].ed - myrinfo->id < 0 && !BetterBalanceKWay(ncon,
                                                                        vwgt + i * ncon,
                                                                        ubfactors,
                                                                        -1,
                                                                        pwgts + from * ncon,
            ...
            if(omode == OMODE_REFINE) {
                ...
                for (j = k - 1; j >= 0; j--) {
                    ...
                    if((mynbrs[j].gv > mynbrs[k].gv
                        ...
                        || (mynbrs[j].gv == mynbrs[k].gv && mynbrs[j].ned > mynbrs[k].ned
                            && ivecaxpylez(ncon, 1, vwgt + i * ncon, pwgts + to * ncon, maxwgt + to * ncon))
                        || (mynbrs[j].gv == mynbrs[k].gv && mynbrs[j].ned == mynbrs[k].ned && BetterBalanceKWay(ncon,
                                                                                                                vwgt + i
                                                                                                                    * ncon,
                                                                                                                ubfactors,
                                                                                                                1,
                ...
                if(xgain + mynbrs[k].gv > 0 || mynbrs[k].ned - myrinfo->nid > 0)
                    ...
                    if((iii % 2 == 0 && safetos[to] == 2) || BetterBalanceKWay(ncon,
                                                                               vwgt + i * ncon,
                                                                               ubfactors,
                                                                               -1,
                                                                               pwgts + from * ncon,
                ...
                if((xgain + mynbrs[k].gv < 0 || (xgain + mynbrs[k].gv == 0 && mynbrs[k].ned - myrinfo->nid < 0))
                    && !BetterBalanceKWay(ncon,
                                          vwgt + i * ncon,
                                          ubfactors,
                                          -1,
...
idx_t IsArticulationNode(idx_t i,
    ...
    for (tnhits = 0, j = xadj[i]; j < xadj[i + 1]; j++) {
        if(where[adjncy[j]] == from) { ASSERT(bfsmrk[adjncy[j]] == 0);ASSERT(bfslvl[adjncy[j]] == 0);
            bfsmrk[k = adjncy[j]] = 1;
            tnhits++;
        }
    }
...
void KWayVolUpdate(ctrl_t* ctrl,
   ...
   *=====================================================================*/
    if(myidx == -1) {
        myidx = myrinfo->nnbrs++;ASSERT(myidx < xadj[v + 1] - xadj[v]);
        mynbrs[myidx].ned = 0;
    }
