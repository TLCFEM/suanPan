...
void FM_2WayCutRefine(ctrl_t* ctrl, graph_t* graph, real_t* ntpwgts, idx_t niter) {
    ...
    for (pass = 0; pass < niter; pass++) { /* Do a number of passes */
        ...
        for (nswaps = 0; nswaps < nvtxs; nswaps++) {
            ...
            IFSET(ctrl->dbglvl,
                  ...
                  printf("Moved %6"PRIDX" from %"PRIDX". [%3"PRIDX" %3"PRIDX"] %5"PRIDX" [%4"PRIDX" %4"PRIDX"]\n", higain, from,
                      ed[higain] - id[higain], vwgt[higain], newcut, pwgts[0], pwgts[1]));
        ...
        *****************************************************************/
        ...
        IFSET(ctrl->dbglvl, METIS_DBG_REFINE, Print2WayRefineStats(ctrl, graph, ntpwgts, 0, mincutorder));
...
void FM_Mc2WayCutRefine(ctrl_t* ctrl, graph_t* graph, real_t* ntpwgts, idx_t niter) {
    idx_t i, ii, j, k, l, kwgt, nvtxs, ncon, nbnd, nswaps, from, to, pass, me, limit, tmp, cnum;
    ...
    IFSET(ctrl->dbglvl, METIS_DBG_REFINE, Print2WayRefineStats(ctrl, graph, ntpwgts, origbal, -2));
    ...
    for (pass = 0; pass < niter; pass++) { /* Do a number of passes */
        ...
        for (nswaps = 0; nswaps < nvtxs; nswaps++) {
            ...
            if((newcut < mincut && newbal <= ffactor) || (newcut == mincut
                && (newbal < minbal || (newbal == minbal && BetterBalance2Way(ncon, minbalv, newbalv))))) {
                mincut = newcut;
                minbal = newbal;
                mincutorder = nswaps;
        ...
        *****************************************************************/
        ...
        IFSET(ctrl->dbglvl, METIS_DBG_REFINE, Print2WayRefineStats(ctrl, graph, ntpwgts, minbal, mincutorder));
    ...
    if(*from != -1) {
        ...
        */
        ...
        for (part = 0; part < 2; part++) {
            for (i = 0; i < ncon; i++) {
                if(rpqLength(queues[2 * i + part]) > 0 && (*from == -1 || rpqSeeTopKey(queues[2 * i + part]) > max)) {
...
void Print2WayRefineStats(ctrl_t* ctrl, graph_t* graph, real_t* ntpwgts, real_t deltabal, idx_t mincutorder) {
    ...
    if(mincutorder == -2) {
        printf("Parts: ");
        printf("Nv-Nb[%5"PRIDX" %5"PRIDX"] ICut: %6"PRIDX, graph->nvtxs, graph->nbnd, graph->mincut);
        printf(" [");
        ...
        printf("\tMincut: %6"PRIDX" at %5"PRIDX" NBND %6"PRIDX" NPwgts: [", graph->mincut, mincutorder, graph->nbnd);
        for (i = 0; i < graph->ncon; i++)
