...
void FM_2WayNodeRefine1SidedP(ctrl_t* ctrl, graph_t* graph, idx_t* hmarker, real_t ubfactor, idx_t npasses) {
    ...
    IFSET(ctrl->dbglvl, METIS_DBG_REFINE, printf("Partitions-N1: [%6"PRIDX" %6"PRIDX"] Nv-Nb[%6"PRIDX" %6"PRIDX"] "
    ...
    for (pass = 0; pass < npasses; pass++) {
        ...
        for (ii = 0; ii < nbnd; ii++) {
            i = bndind[swaps[ii]];ASSERT(where[i] == 2);
            ...
            ***********************************************************/
            ...
            IFSET(ctrl->dbglvl,
                  ...
                  printf("Moved %6"PRIDX" to %3"PRIDX", Gain: %5"PRIDX" [%5"PRIDX"] \t[%5"PRIDX" %5"PRIDX" %5"PRIDX"] [%3"PRIDX" %2"PRIDX"]\n", higain, to, (
                      vwgt[higain]
                          - rinfo[higain].edegrees[from]), vwgt[higain], pwgts[0], pwgts[1], pwgts[2], nswaps, limit));
        ...
        *******************************************************/
        ...
        for (nswaps = 0; nswaps < nvtxs; nswaps++) {
            ...
            ***********************************************************/
            ...
            IFSET(ctrl->dbglvl, METIS_DBG_MOVEINFO, printf("Moved %6"PRIDX" to %3"PRIDX", Gain: %5"PRIDX" [%5"PRIDX"] "
