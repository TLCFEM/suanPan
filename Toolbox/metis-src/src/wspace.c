...
void AllocateWorkSpace(ctrl_t* ctrl, graph_t* graph) {
    ...
    case METIS_OP_PMETIS:
        coresize = 3 * (graph->nvtxs + 1) * sizeof(idx_t) + 5 * (ctrl->nparts + 1) * graph->ncon * sizeof(idx_t)
            + 5 * (ctrl->nparts + 1) * graph->ncon * sizeof(real_t);
        break;
    default:
        coresize = 4 * (graph->nvtxs + 1) * sizeof(idx_t) + 5 * (ctrl->nparts + 1) * graph->ncon * sizeof(idx_t)
            + 5 * (ctrl->nparts + 1) * graph->ncon * sizeof(real_t);
    }
    /*coresize = 0;*/
...
void AllocateRefinementWorkSpace(ctrl_t* ctrl, idx_t nbrpoolsize) {
    ...
    case METIS_OBJTYPE_CUT:
        ctrl->cnbrpool =
            (cnbr_t*) gk_malloc(ctrl->nbrpoolsize * sizeof(cnbr_t), "AllocateRefinementWorkSpace: cnbrpool");
        break;

    case METIS_OBJTYPE_VOL:
        ctrl->vnbrpool =
            (vnbr_t*) gk_malloc(ctrl->nbrpoolsize * sizeof(vnbr_t), "AllocateRefinementWorkSpace: vnbrpool");
        break;
...
void FreeWorkSpace(ctrl_t* ctrl) {
    ...
    IFSET(ctrl->dbglvl, METIS_DBG_INFO, printf(" nbrpool statistics\n"
...
idx_t cnbrpoolGetNext(ctrl_t* ctrl, idx_t nnbrs) {
    ...
    if(ctrl->nbrpoolcpos > ctrl->nbrpoolsize) {
        ...
        ctrl->cnbrpool =
            (cnbr_t*) gk_realloc(ctrl->cnbrpool, ctrl->nbrpoolsize * sizeof(cnbr_t), "cnbrpoolGet: cnbrpool");
        ctrl->nbrpoolreallocs++;
    }
...
idx_t vnbrpoolGetNext(ctrl_t* ctrl, idx_t nnbrs) {
    ...
    if(ctrl->nbrpoolcpos > ctrl->nbrpoolsize) {
        ...
        ctrl->vnbrpool =
            (vnbr_t*) gk_realloc(ctrl->vnbrpool, ctrl->nbrpoolsize * sizeof(vnbr_t), "vnbrpoolGet: vnbrpool");
        ctrl->nbrpoolreallocs++;
    }
