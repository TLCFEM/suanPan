...
void ComputeBFSOrdering(ctrl_t* ctrl, graph_t* graph, idx_t* bfsperm) {
    ...
    while (first < nvtxs) {
        if(first == last) { /* Find another starting vertex */
            k = bfsperm[last];ASSERT(perm[k] != -1);
...
void EliminateComponents(ctrl_t* ctrl, graph_t* graph) {
    ...
    if(ncmps > nparts) {
        ...
        for (ntodo = 0, i = 0; i < nparts; i++) {
            ...
            for (j = cptr[bestcid]; j < cptr[bestcid + 1]; j++) { ASSERT(where[cind[j]] == i);
                cwhere[cind[j]] = i;
            }
        }
        ...
        while (ntodo > 0) {
            ...
            for (i = 0; i < ntodo; i++) {
                ...
                if(target != me) {
                    ...
                    case METIS_OBJTYPE_VOL: MoveGroupContigForVol(ctrl,
                                                                  graph,
                                                                  target,
                                                                  cid,
                                                                  cptr,
        ...
        for (i = 0; i < nvtxs; i++)ASSERT(where[i] == cwhere[i]);
