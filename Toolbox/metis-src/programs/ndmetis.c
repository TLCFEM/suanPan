...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    status = METIS_NodeND(&graph->nvtxs, graph->xadj, graph->adjncy, graph->vwgt, options, perm, iperm);
    ...
    gk_free((void**) &params->filename, &params->tpwgtsfile, &params->tpwgts, &params->ubvec, &params, LTERM);
...
void NDPrintInfo(params_t* params, graph_t* graph) {
    ...
    printf(" ctype=%s, rtype=%s, iptype=%s, seed=%"PRIDX", dbglvl=%"PRIDX"\n",
           ctypenames[params->ctype],
           rtypenames[params->rtype],
