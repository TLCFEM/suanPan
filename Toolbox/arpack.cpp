...
int eig_psolve(cx_vec& eigval, cx_mat& eigvec, const mat_ptr& K, const mat_ptr& M, const unsigned num, const char* WHICH) {
    static auto BMAT{'G'}; // generalized eigenvalue problem A*x=lambda*B*x
    ...
    while(99 != IDO) {
        pdnaupd_(&COMM, &IDO, &BMAT, &N, WHICH, &NEV, &TOL, RESID.memptr(), &NCV, V.memptr(), &N, IPARAM, IPNTR, WORKD.memptr(), WORKL.memptr(), &LWORKL, &INFO);
        if(0 != INFO) break;
        // ReSharper disable once CppEntityAssignedButNoRead
        if(vec Y(WORKD.memptr() + IPNTR[1] - 1, N, false, true); -1 == IDO) {
            vec X(WORKD.memptr() + IPNTR[0] - 1, N, false, true);
    ...
    pdneupd_(&COMM, &RVEC, &HOWMNY, SELECT.memptr(), DR.memptr(), DI.memptr(), Z.memptr(), &N, &SIGMA, &SIGMA, WORKEV.memptr(), &BMAT, &N, WHICH, &NEV, &TOL, RESID.memptr(), &NCV, V.memptr(), &N, IPARAM, IPNTR, WORKD.memptr(), WORKL.memptr(), &LWORKL, &INFO);
...
int eig_psolve(cx_vec& eigval, const std::shared_ptr<MetaMat<double>>& K, const unsigned num) {
    static auto BMAT{'I'};              // standard eigenvalue problem A*x=lambda*x
    static constexpr auto WHICH = "SR"; // smallest real part
    ...
    pdneupd_(&COMM, &RVEC, &HOWMNY, SELECT.memptr(), DR.memptr(), DI.memptr(), nullptr, &N, &SIGMA, &SIGMA, WORKEV.memptr(), &BMAT, &N, WHICH, &NEV, &TOL, RESID.memptr(), &NCV, V.memptr(), &N, IPARAM, IPNTR, WORKD.memptr(), WORKL.memptr(), &LWORKL, &INFO);
