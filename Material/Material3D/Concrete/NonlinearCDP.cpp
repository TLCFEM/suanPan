...
int NonlinearCDP::update_trial_status(const vec& t_strain) {
    ...
    auto dgdsigma_t = (pn(2) + alpha_p) / g_t;
    auto dgdsigma_c = (pn(0) + alpha_p) / g_c;

    const auto evolve_kappa_t = dgdsigma_t > 0.;
    const auto evolve_kappa_c = dgdsigma_c < 0.;

    if(!evolve_kappa_t) dgdsigma_t = .0;
    ...
    if(evolve_kappa_t) left.row(1) = t_para[1] * lambda * (r / g_t * trans.row(2) * dnde + dgdsigma_t * prpe);
