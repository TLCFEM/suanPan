...
int DafaliasManzari::update_trial_status(const vec& t_strain) {
    ...
    while(true) {
        ...
        SP_D("Local elastic iteration error: {:.5E}.\n", error /= ref_error);
    ...
    while(true) {
        ...
        auto error = norm(residual);
        if(1 == counter) ref_error = std::max(1., error);
        SP_D("Local plastic iteration error: {:.5E}.\n", error /= ref_error);
        if(error <= tolerance) break;
...
void DafaliasManzari::print() { sp_info("A Dafalias-Manzari sand model. doi: 10.1061/(ASCE)0733-9399(2004)130:6(622)\n"); }
