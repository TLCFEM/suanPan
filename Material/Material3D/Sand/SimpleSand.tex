...
The state parameter is taken from \href{https://doi.org/10.1016/j.cma.2005.08.020}{10.1016/j.cma.2005.08.020}. Other parts are mostly taken from \href{https://doi.org/10.1061/(ASCE)0733-9399(2004)130:6(622)}{10.1061/(ASCE)0733-9399(2004)130:6(622)}. The Lode angle dependency is removed. The fabric effect is removed.
\section{Elasticity}
...
\begin{gather*}
\Delta\mathbold{\varepsilon}^p=\gamma\left(\mathbold{n}+\dfrac{1}{3}A\left(\alpha^d-m-\mathbold{\alpha}:\mathbold{n}\right)\mathbold{I}\right),
\end{gather*}
where $D=A\left(\alpha^d-m-\mathbold{\alpha}:\mathbold{n}\right)$ is the dilatancy parameter, $A$ is a model constant, it can also be defined as a function of current state. Note since the sign convention is changed, a positive $A$ leads to dilatant behaviour. Often it needs to be negative.
...
p=p^{tr}-KA\gamma\left(\alpha^d-m-\mathbold{\alpha}:\mathbold{n}\right).
\end{gather*}
...
The evolution rate of the back stress ratio $\mathbold{\alpha}$ is defined in terms of a proper distance measure from the bounding surface, Here, such a distance measure is chosen to be $h\left(\left(\alpha^b-m\right)\mathbold{n}-\mathbold{\alpha}\right)$, where $h$ is a model constant. Thus,
\begin{gather*}
...
|\mathbold{\eta}|+mp,\\[3mm]
p-p^{tr}+KA\gamma\left(\alpha^d-m-\mathbold{\alpha}:\mathbold{n}\right),\\[3mm]
\mathbold{s}-\mathbold{s}^{tr}+2G\gamma\mathbold{n},\\[3mm]
\mathbold{\alpha}_n+\gamma{}h\left(\alpha^b-m\right)\mathbold{n}-\left(\gamma{}h+1\right)\mathbold{\alpha}.
\end{array}\right.
\end{gather*}
...
\begin{bmatrix}
	...
	KA\left(\alpha^d-m-\mathbold{\alpha}:\mathbold{n}\right) & 1+KA\gamma\left(\pfrac{\alpha^d}{p}-\mathbold{\alpha}:\pfrac{\mathbold{n}}{p}\right)  & -KA\gamma\mathbold{\alpha}:\pfrac{\mathbold{n}}{\mathbold{s}} & -KA\gamma\left(\mathbf{I}:\mathbold{n}+\mathbold{\alpha}:\pfrac{\mathbold{n}}{\mathbold{\alpha}}\right) \\[4mm]
	...
	h\left(\alpha^b-m\right)\mathbold{n}-h\mathbold{\alpha}               & \gamma{}h\left(\pfrac{\alpha^b}{p}\mathbold{n}+\left(\alpha^b-m\right)\pfrac{\mathbold{n}}{p}\right) & \gamma{}h\left(\alpha^b-m\right)\pfrac{\mathbold{n}}{\mathbold{s}}           & \gamma{}h\left(\alpha^b-m\right)\pfrac{\mathbold{n}}{\mathbold{\alpha}}-\left(\gamma{}h+1\right)\mathbf{I}
\end{bmatrix}.
\end{gather*}
\end{footnotesize}
...
\begin{bmatrix}
	...
	h\left(\alpha^b-m\right)\mathbold{n}-h\mathbold{\alpha}               & \gamma{}h\left(\pfrac{\alpha^b}{p}\mathbold{n}+\dfrac{\alpha^b-m}{|\mathbold{\eta}|}\left(\mathbold{\alpha}-\left(\mathbold{n}:\mathbold{\alpha}\right)\cdot\mathbold{n}\right)\right) & \dfrac{\gamma{}h\left(\alpha^b-m\right)}{|\mathbold{\eta}|}\left(\mathbf{I}-\mathbold{n}\otimes\left(\mathbold{n}:\mathbf{I}\right)\right)               & \dfrac{\gamma{}h\left(\alpha^b-m\right)p}{|\mathbold{\eta}|}\left(\mathbf{I}-\mathbold{n}\otimes\left(\mathbold{n}:\mathbf{I}\right)\right)-\left(\gamma{}h+1\right)\mathbf{I}
\end{bmatrix}.
\end{gather*}
\end{tiny}
...
\begin{bmatrix}
	...
	-2G\mathbf{I}_{d}                                       \\[3mm]
	-\alpha^bn^bv_0\gamma{}h\mathbold{n}\otimes\mathbold{I}
\end{bmatrix}.
\end{gather*}
Thus,
\begin{gather*}
