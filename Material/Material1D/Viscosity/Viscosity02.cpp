...
Viscosity02::Viscosity02(const unsigned T, const double A, const double CA, const double CB, const double CC, const double CD, const double GA, const double GB, const double L)
    : DataViscosity02{.damping_a = .25 * (std::fabs(CA) + std::fabs(CB) + std::fabs(CC) + std::fabs(CD)), .damping_b = .5 / datum::pi * (std::fabs(CA) - std::fabs(CB) - std::fabs(CC) + std::fabs(CD)), .damping_c = .5 / datum::pi * (std::fabs(CA) + std::fabs(CB) - std::fabs(CC) - std::fabs(CD)), .damping_d = (std::fabs(CA) - std::fabs(CB) + std::fabs(CC) - std::fabs(CD)) / datum::pi / datum::pi, .gap_a = std::fabs(GA), .gap_b = std::fabs(GB)}
    , NonlinearViscosity(T, std::fabs(A), std::fabs(L)) {}

unique_ptr<Material> Viscosity02::unique_copy() { return std::make_unique<Viscosity02>(*this); }
