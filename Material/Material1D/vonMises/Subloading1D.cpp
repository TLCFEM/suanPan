...
std::tuple<double, double> Subloading1D::isotropic_bound(const double q) const {
...
std::tuple<double, double> Subloading1D::kinematic_bound(const double q) const {
...
int Subloading1D::pure_plastic(vec& start_history) {
...
int Subloading1D::partial_unloading(double& fragment, vec& start_history) {
    const auto& current_q = start_history(1);
    const auto& current_z = start_history(2);
    const vec current_alpha(&start_history(4), b.size(), false, true);
    const vec current_d(&start_history(4 + b.size()), c.size(), false, true);
