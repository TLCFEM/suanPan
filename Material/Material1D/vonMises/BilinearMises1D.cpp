...
BilinearMises1D::BilinearMises1D(const unsigned T, const double E, const double Y, const double H, const double R)
    : DataBilinearMises1D{.yield_stress = std::fabs(Y), .isotropic_modulus = std::fabs(E) * H / (1. - H)}
    , NonlinearMises1D(T, E, R) {}

unique_ptr<Material> BilinearMises1D::unique_copy() { return std::make_unique<BilinearMises1D>(*this); }
