...
pod2 ConcreteTsai::compute_compression_backbone(const double n_strain) const {
    ...
    response[1] = elastic_modulus * (1. - tmp_a) / tmp_b / tmp_b;
...
double ConcreteTsai::compute_compression_residual(const double reverse_c_strain, const double reverse_c_stress) const { return std::min(0., reverse_c_strain - reverse_c_stress * (reverse_c_strain / c_strain + .57) / (reverse_c_stress / c_strain + .57 * elastic_modulus)); }

double ConcreteTsai::compute_tension_residual(const double reverse_t_strain, const double reverse_t_stress) const { return std::max(0., reverse_t_strain - reverse_t_stress * (reverse_t_strain / t_strain + .67) / (reverse_t_stress / t_strain + .67 * elastic_modulus)); }
...
int ConcreteTsai::initialize(const shared_ptr<DomainBase>& D) {
