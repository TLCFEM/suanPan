...
public:
    ...
    void nullify(const uword K) override {
        this->factored = false;
        suanpan::for_each(0llu, K, [&](const uword I) { this->memory[K + (length - I) * I / 2] = T(0); });
...
template<sp_d T> Mat<T> SymmPackMat<T>::operator*(const Mat<T>& X) const {
    ...
    if constexpr(std::is_same_v<T, float>) {
        using E = float;
        suanpan::for_each(0llu, X.n_cols, [&](const uword I) { arma_fortran(arma_sspmv)(&UPLO, &N, (E*)&ALPHA, (E*)this->memptr(), (E*)X.colptr(I), &INC, (E*)&BETA, (E*)Y.colptr(I), &INC); });
    }
    else {
        using E = double;
        suanpan::for_each(0llu, X.n_cols, [&](const uword I) { arma_fortran(arma_dspmv)(&UPLO, &N, (E*)&ALPHA, (E*)this->memptr(), (E*)X.colptr(I), &INC, (E*)&BETA, (E*)Y.colptr(I), &INC); });
    }
