...
template<typename T> void DenseMat<T>::init() {
	if(nullptr != memory) memory::release(access::rw(memory));
	access::rw(memory) = is_empty() ? nullptr : memory::acquire<T>(this->n_elem);
}

template<typename T> podarray<float> DenseMat<T>::to_float() {
	podarray<float> f_memory(this->n_elem);

#ifdef SUANPAN_MT
	tbb::parallel_for(0llu, this->n_elem, [&](const uword I) { f_memory(I) = static_cast<float>(memory[I]); });
#else
	std::transform(memory, memory + this->n_elem, f_memory.mem, [](const double I) { return static_cast<float>(I); });
#endif

	return f_memory;
}

template<typename T> DenseMat<T>::DenseMat(const uword in_rows, const uword in_cols, const uword in_elem)
...
template<typename T> DenseMat<T>::~DenseMat() { if(nullptr != memory) memory::release(access::rw(memory)); }

template<typename T> bool DenseMat<T>::is_empty() const { return 0 == this->n_elem; }
...
template<typename T> const T* DenseMat<T>::memptr() const { return memory; }

template<typename T> T* DenseMat<T>::memptr() { return const_cast<T*>(memory); }
...
template<typename T> int DenseMat<T>::solve(Mat<T>& X, const SpMat<T>& B) { return this->solve(X, Mat<T>(B)); }

template<typename T> int DenseMat<T>::solve(Mat<T>& X, Mat<T>&& B) { return this->solve(X, B); }

template<typename T> int DenseMat<T>::solve(Mat<T>& X, SpMat<T>&& B) { return this->solve(X, Mat<T>(B)); }

template<typename T> int DenseMat<T>::sign_det() const {
	auto det_sign = 1;
