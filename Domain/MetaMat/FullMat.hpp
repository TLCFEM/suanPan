...
template<typename T> class FullMat : public DenseMat<T> {
...
template<typename T> unique_ptr<MetaMat<T>> FullMat<T>::make_copy() { return std::make_unique<FullMat<T>>(*this); }

template<typename T> void FullMat<T>::unify(const uword K) {
#ifdef SUANPAN_MT
	tbb::parallel_for(0llu, this->n_rows, [&](const uword I) { at(I, K) = 0.; });
	tbb::parallel_for(0llu, this->n_cols, [&](const uword I) { at(K, I) = 0.; });
#else
	for(auto I = 0llu; I < this->n_rows; ++I) at(I, K) = 0.;
	for(auto I = 0llu; I < this->n_cols; ++I) at(K, I) = 0.;
#endif
	at(K, K) = 1.;
}
...
template<typename T> int FullMat<T>::solve_trs(Mat<T>& X, Mat<T>&& B) {
	auto N = static_cast<int>(this->n_rows);
	auto NRHS = static_cast<int>(B.n_cols);
	auto LDB = static_cast<int>(B.n_rows);
	auto INFO = 0;
