...
template<typename T> class BandMat final : public DenseMat<T> {
...
template<typename T> unique_ptr<MetaMat<T>> BandMat<T>::make_copy() { return std::make_unique<BandMat<T>>(*this); }

template<typename T> void BandMat<T>::unify(const uword K) {
#ifdef SUANPAN_MT
	tbb::parallel_for(std::max(K, u_band) - u_band, std::min(this->n_rows, K + l_band + 1), [&](const uword I) { access::rw(this->memory[I + s_band + K * (m_rows - 1)]) = 0.; });
	tbb::parallel_for(std::max(K, l_band) - l_band, std::min(this->n_cols, K + u_band + 1), [&](const uword I) { access::rw(this->memory[K + s_band + I * (m_rows - 1)]) = 0.; });
#else
	for(auto I = std::max(K, u_band) - u_band; I < std::min(this->n_rows, K + l_band + 1); ++I) access::rw(this->memory[I + s_band + K * (m_rows - 1)]) = 0.;
	for(auto I = std::max(K, l_band) - l_band; I < std::min(this->n_cols, K + u_band + 1); ++I) access::rw(this->memory[K + s_band + I * (m_rows - 1)]) = 0.;
#endif
