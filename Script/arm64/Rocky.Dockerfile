...
RUN git clone --recurse-submodules --depth 1 --branch dev https://github.com/TLCFEM/suanPan.git && cd suanPan && mkdir build && cd build && \
    cmake -DCMAKE_BUILD_TYPE=Release -DSP_ENABLE_SYSLIB=ON -DSP_OPENBLAS_PATH=/OpenBLAS/libopenblas.a -DSP_BUILD_PARALLEL=ON -DSP_ENABLE_HDF5=ON -DSP_ENABLE_VTK=ON -DSP_ENABLE_AVX2=OFF .. && make -j"$(nproc)"
