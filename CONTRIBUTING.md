...
Contributions are welcome. Feature requests are welcome, please email me at `tlcfem(at)gmail(dot)com`. If needed, additional help/collaboration can be provided.
...
2. Please use `CamelCase` for class names. Abbreviations shall be in upper case, for example `NURBSSurface` instead of `NurbsSurface`.
3. Please use meaningful variable names instead of abstract names such as `a`, `tt`, etc.
4. Please avoid abbreviations, except for common ones such as `ptr` and `tmp`. For variables related to the corresponding theories, they can be spelled out. It does not hurt to define `epsilon_vol` instead of `ev` to represent volumetric strain variable. This is for the ease of readability and maintenance.
...
5. Please always provide an example model that tests the code and documentation.
6. Please check similar implemented code before getting started.
7. Please provide unit tests ([Catch2](https://github.com/catchorg/Catch2)) if applicable.
