...
int VAFNM::compute_local_integration(vec& q, mat& jacobian) {
    ...
    const auto &ani = current_history(d_size), &anj = current_history(d_size + 1u);

    vec beta(&trial_history(0), d_size, false, true);
    auto &ai = trial_history(d_size), &aj = trial_history(d_size + 1u);
    auto &flagi = trial_history(d_size + 2u), &flagj = trial_history(d_size + 3u); // yield flag
