...
#include "BoundingNM.h"
#include <Toolbox/utility.h>

int BoundingNM::compute_local_integration(vec& q, mat& jacobian, const bool yield_flagi, const bool yield_flagj) {
    ...
    while(true) {
        if(max_iteration == ++counter) {
            suanpan_error("BoundingNM cannot converge within %u iterations.\n", max_iteration);
            return SUANPAN_FAIL;
        }
        ...
        const mat tmp_a = bounding_saturation * gamma * beta * m.t();
        ...
        mat prpe(g_size, d_size, fill::zeros), dedx(d_size, g_size, fill::zeros);
        ...
        prpe.rows(ga) = eye(d_size, d_size);
        ...
        if(has_kinematic) {
            ...
            jacobian(gb, gb) += bounding_saturation * gamma * norm_z * eye(d_size, d_size) - tmp_a * pzpq;
            jacobian(gb, gc) = tmp_a * pzpai;
            jacobian(gb, gd) = tmp_a * pzpaj;
            jacobian(gb, ge) = bounding_saturation * norm_z * beta;

            jacobian(gc, gb) = tzi * pzpq;
            jacobian(gd, gb) = tzj * pzpq;
            jacobian(ge, gb) = -z.t();

            residual(gb) = (1. + bounding_saturation * gamma * norm_z) * beta - current_beta - bounding_rate * gamma * z;

            prpe.rows(gb) = -bounding_rate * eye(d_size, d_size);
        ...
        suanpan_debug("BoundingNM local iteration error: %.5E.\n", error /= ref_error);
...
vec BoundingNM::compute_h(const double alpha) const { return {n_size, fill::value(std::max(datum::eps, 1. + iso_modulus * alpha + iso_saturation - iso_saturation * exp(-iso_decay * alpha)))}; }

vec BoundingNM::compute_dh(const double alpha) const { return compute_h(alpha).transform([&](const double h) { return suanpan::approx_equal(h, datum::eps) ? 0. : iso_modulus + iso_saturation * iso_decay * exp(-iso_decay * alpha); }); }

BoundingNM::BoundingNM(const unsigned T, const double EEA, const double EEIS, const double HH, const double HS, const double HD, const double BS, const double BR, const double LD, vec&& YF)
    : NonlinearNM(T, EEA, EEIS, !suanpan::approx_equal(BR, 0.), LD, std::forward<vec>(YF))
    , iso_modulus(HH)
    , iso_saturation(HS)
    , iso_decay(HD)
    , bounding_saturation(BS)
    , bounding_rate(BR) {}

BoundingNM::BoundingNM(const unsigned T, const double EEA, const double EEIS, const double EEIW, const double HH, const double HS, const double HD, const double BS, const double BR, const double LD, vec&& YF)
    : NonlinearNM(T, EEA, EEIS, EEIW, !suanpan::approx_equal(BR, 0.), LD, std::forward<vec>(YF))
    , iso_modulus(HH)
    , iso_saturation(HS)
    , iso_decay(HD)
    , bounding_saturation(BS)
